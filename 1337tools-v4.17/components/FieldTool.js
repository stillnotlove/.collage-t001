...
export default function FieldTool({onIndex,onSendToEditor,onContinue,trail=[],initialFile,initialRatio='4 / 5',initialToken}){
  ...
  async function loadFile(file){
    ...
    img.onerror=()=>{if(urlRef.current===url)urlRef.current=null;URL.revokeObjectURL(url);reportToolError('FIELD image load',new Error('Unsupported or damaged image'))};
    img.src=url;
  }
  ...
  function render(ctx,w,h,{clean=false,transparent=false}={}){
    ...
    for(let gy=0;gy<rows&&drawn<count;gy++)for(let gx=0;gx<cols&&drawn<count;gx++,drawn++){
      ...
      ctx.save();ctx.translate(x,y);ctx.rotate(angle);ctx.globalAlpha=alpha;ctx.globalCompositeOperation=blendMode(fx.blend);ctx.drawImage(src,-iw/2,-ih/2,iw,ih);ctx.restore();
  ...
  },[source,params,fx,seed,point,ratio]);
  ...
  async function sendToEditor(){if(!imgRef.current||exporting)return;setExporting(true);try{const blob=await renderBlob({transparent:true}),file=new File([blob],`FIELD-${Date.now()}.png`,{type:'image/png'});onSendToEditor?.(file,ratio)}catch(err){reportToolError('FIELD → EDITOR',err)}finally{setExporting(false)}}
  async function continueTo(target){if(!imgRef.current||exporting)return;setExporting(true);try{const blob=await renderBlob({transparent:true}),file=new File([blob],`FIELD-${target}-${Date.now()}.png`,{type:'image/png'});onContinue?.('field',target,file,ratio)}catch(err){reportToolError(`FIELD → ${String(target).toUpperCase()}`,err)}finally{setExporting(false)}}

  return <main className="fieldShell fieldShell41">
    <aside className="fieldHud">
      <div className="brand"><span className="brand1337">1337</span><span className="brandTools">tools</span><small>4.17 / 01 FIELD</small></div>
      ...
      <section><div className="title">OUTPUT</div><button className="wide primary" disabled={!source||exporting} onClick={sendToEditor}>SEND TO EDITOR</button><ContinuePanel current="field" disabled={!source||exporting} onChoose={continueTo}/><div className="two"><button disabled={!source||exporting} onClick={()=>exportPng(false)}>{exporting?'…':'PNG'}</button><button disabled={!source||exporting} onClick={()=>exportPng(true)}>{exporting?'…':'PNG α'}</button></div><div className="note">Continue passes the rendered output forward as transparent PNG.</div></section>
    </aside>
