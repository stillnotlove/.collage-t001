...
check(shell.includes('<span>4.54</span>'),'INDEX: version is not 4.54');
check((shell.match(/4\.54/g)||[]).length===1,'INDEX: version should appear only once in AppShell');
...
check(entry.includes('const ASCII_STYLES=['),'ENTRY: ASCII style pool missing');
check(entry.includes('const SLICE_PATTERNS=['),'ENTRY: SLICE pattern pool missing');
check(entry.includes('const ECHO_VARIANTS=['),'ENTRY: ECHO variant pool missing');
check(entry.includes('const ROUTE_TEMPLATES=['),'ENTRY: randomized route templates missing');
check(entry.includes('clean → 3 generated effects → clean')||entry.includes("[{kind:'clean'},...template.map"),'ENTRY: clean route grammar missing');
check(entry.includes('function buildRoute()'),'ENTRY: rebuilt route engine missing');
check(entry.includes("segments.push({type:'hold'"),'ENTRY: timeline hold segments missing');
check(entry.includes("segments.push({type:'transition'"),'ENTRY: timeline transition segments missing');
check(entry.includes('function renderTransition('),'ENTRY: normalized transition renderer missing');
check(entry.includes('compositeStripReveal'),'ENTRY: ASCII strip reveal compositor missing');
check(entry.includes('compositeMotionBlend'),'ENTRY: motion blend compositor missing');
check(entry.includes('compositeSoftMix'),'ENTRY: soft mix compositor missing');
check(entry.includes('renderLiveNodeFrame'),'ENTRY: live transition frame rendering missing');
check(entry.includes('while(segmentIndex<route.segments.length-1&&elapsed>=route.segments[segmentIndex].end)'),'ENTRY: segment progression guard missing');
check(entry.includes('visibilitychange'),'ENTRY: visibility pause/resume guard missing');
check(entry.includes('ResizeObserver'),'ENTRY: resize observer guard missing');
check(!entry.includes('renderSliceIntoAscii'),'ENTRY: stale per-transition slice->ascii compositor still present');
check(!entry.includes('renderEchoIntoAscii'),'ENTRY: stale per-transition echo->ascii compositor still present');
check(!entry.includes('overlapOutOfAscii'),'ENTRY: stale ASCII-out compositor still present');
...
check(visualAudit.includes('<span>4.54</span>'),'QA: static visual audit has stale version');
...
check(pkg.version==='0.4.54','package.json: stale version');
