...
export default function CollageEditor(){
  ...
  const canvas=useRef(null),fileRef=useRef(null),gesture=useRef(null),before=useRef(null),brush=useRef(null),variationBase=useRef(null),controlBefore=useRef(null),ownedUrls=useRef(new Set()),keyActions=useRef(null),marqueeLive=useRef(null),suppressCanvasClick=useRef(false);
  ...
  return <main className="shell">
    <aside className="panel left">
      <div className="brand" aria-label="1337tools"><span className="brand1337">1337</span><span className="brandTools">tools</span><small>3.9</small></div>
    ...
    <aside className="panel right">
      ...
      {!selected?<div className="muted">Select an object. Drag an empty canvas area for marquee selection. Shift adds, Alt removes.</div>:<>
        ...
        {selectedIds.length===1&&<section><Title>Repeat / Pattern</Title><button className="wide primary randomHero" onClick={randomRepeat}>🎲 Random repeat</button><button className="wide" onClick={()=>repeatSelected()}>Repeat current</button><details className="advanced"><summary>Manual controls</summary><Range label="Count" min="1" max="40" value={repeat.count} onChange={v=>setRepeat({...repeat,count:+v})}/><Range label="Spacing" min="-20" max="20" value={repeat.spacing} onChange={v=>setRepeat({...repeat,spacing:+v})}/><Range label="Angle step" min="-45" max="45" value={repeat.angle} onChange={v=>setRepeat({...repeat,angle:+v})}/><Range label="Scale drift" min="-80" max="150" value={repeat.scale} onChange={v=>setRepeat({...repeat,scale:+v})}/><Range label="Opacity drift" min="0" max="95" value={repeat.opacity} onChange={v=>setRepeat({...repeat,opacity:+v})}/><Range label="Order ↔ Chaos" min="0" max="100" value={repeat.chaos} onChange={v=>setRepeat({...repeat,chaos:+v})}/></details></section>}
