...
export default function CollageEditor(){
  ...
  const canvas=useRef(null),fileRef=useRef(null),gesture=useRef(null),before=useRef(null),brush=useRef(null),variationBase=useRef(null),controlBefore=useRef(null),ownedUrls=useRef(new Set()),keyActions=useRef(null);
  ...
  function rerollAll(){
    ...
    const nextFx=f=>({...f,threshold:Math.round(Math.random()*55),grain:Math.round(Math.random()*40),noise:Math.round(8+Math.random()*55),halftone:Math.round(Math.random()*85),halftoneSize:Math.round(6+Math.random()*20),halftoneDensity:Math.round(20+Math.random()*70),halftoneOpacity:Math.round(40+Math.random()*60),halftoneAngle:Math.round(-45+Math.random()*90),halftoneColor:Math.random()>.65?'#ffffff':'#000000',xerox:Math.round(Math.random()*45),fade:Math.round(Math.random()*28)});
    commit(layers.map(l=>l.id===selected.id?{...l,fxSeed:Math.floor(Math.random()*1e9),fx:nextFx({...fx0,...l.fx,scratches:0})}:l),selectedIds,primaryId);
  }
  ...
  return <main className="shell">
    <aside className="panel left">
      <div className="brand" aria-label="1337tools"><span className="brand1337">1337</span><span className="brandTools">tools</span><small>3.7</small></div>
    ...
    <aside className="panel right">
      ...
      {!selected?<div className="muted">Select an object. Drag an empty canvas area for marquee selection. Shift adds, Alt removes.</div>:<>
        {selectedIds.length>1?<>
          ...
          {imageSelection.length>0&&<section><Title>Group Effects · images</Title><div className="muted">Effects apply to image layers ({imageSelection.length}/{selectedIds.length}).</div>{imageEffectKeys.map(k=><Range key={k} label={k} min="0" max="100" value={Math.round(({...fx0,...primaryImage?.fx}[k])||0)} onBegin={beginControl} onEnd={endControl} onChange={v=>patchSelectedLive(l=>l.type==='image'?{...l,fx:{...fx0,...l.fx,[k]:+v,scratches:0}}:l)}/>)}</section>}
        ...
        {selectedIds.length===1&&selected.type==='image'&&<ImageLab s={selected} patch={patchLive} patchFx={patchFxLive} begin={beginControl} end={endControl} cutting={cutting} cutout={cutout} restore={restoreBackground} reroll={rerollTexture} rerollAll={rerollAll}/>} 
        {selectedIds.length===1&&<section><Title>Repeat / Pattern</Title><div className="two"><button onClick={randomizeRepeat}>Randomize</button><button className="primary" onClick={repeatSelected}>Repeat selected</button></div><Range label="Count" min="1" max="40" value={repeat.count} onChange={v=>setRepeat({...repeat,count:+v})}/><Range label="Spacing" min="-20" max="20" value={repeat.spacing} onChange={v=>setRepeat({...repeat,spacing:+v})}/><Range label="Angle step" min="-45" max="45" value={repeat.angle} onChange={v=>setRepeat({...repeat,angle:+v})}/><Range label="Scale drift" min="-80" max="150" value={repeat.scale} onChange={v=>setRepeat({...repeat,scale:+v})}/><Range label="Opacity drift" min="0" max="95" value={repeat.opacity} onChange={v=>setRepeat({...repeat,opacity:+v})}/><Range label="Order ↔ Chaos" min="0" max="100" value={repeat.chaos} onChange={v=>setRepeat({...repeat,chaos:+v})}/></section>}
        <section className="actions"><button onClick={front}>Front</button><button onClick={back}>Back</button><button onClick={()=>moveSelectionStep(1)}>Forward</button><button onClick={()=>moveSelectionStep(-1)}>Backward</button><button onClick={duplicate}>Duplicate</button><button className="danger" onClick={remove}>Delete</button></section>
      </>}
    </aside>
  </main>;
}

function ImageLab({s,patch,patchFx,begin,end,cutting,cutout,restore,reroll,rerollAll}){
  const f={...fx0,...s.fx};
  return <section><Title>Image Lab</Title><label>Fit<select value={s.fit} onFocus={begin} onBlur={end} onChange={e=>patch(s.id,{fit:e.target.value})}><option>cover</option><option>contain</option><option>fill</option></select></label><Range label="Corner radius" min="0" max="50" value={s.radius} onBegin={begin} onEnd={end} onChange={v=>patch(s.id,{radius:+v})}/><div className="two"><button className="primary" disabled={cutting} onClick={cutout}>{cutting?'Cutting…':s.cutout?'Re-cut β':'Cutout β'}</button><button disabled={!s.originalSrc||!s.cutout} onClick={restore}>Restore</button></div><div className="note">Cutout β is intentionally experimental. Ctrl/Cmd+V pastes images from the clipboard.</div><h4>ADJUST</h4><Range label="Exposure" min="-50" max="50" value={f.exposure} onBegin={begin} onEnd={end} onChange={v=>patchFx(s.id,'exposure',+v)}/><Range label="Contrast" min="0" max="300" value={f.contrast} onBegin={begin} onEnd={end} onChange={v=>patchFx(s.id,'contrast',+v)}/><Range label="Saturation" min="0" max="250" value={f.saturation} onBegin={begin} onEnd={end} onChange={v=>patchFx(s.id,'saturation',+v)}/><Range label="Grayscale" min="0" max="100" value={f.grayscale} onBegin={begin} onEnd={end} onChange={v=>patchFx(s.id,'grayscale',+v)}/><Range label="Blur" min="0" max="16" value={f.blur} onBegin={begin} onEnd={end} onChange={v=>patchFx(s.id,'blur',+v)}/><h4>DISTRESS</h4><div className="two"><button onClick={reroll}>Reroll seed</button><button className="primary" onClick={rerollAll}>REROLL ALL</button></div><Range label="Threshold" min="0" max="100" value={f.threshold} onBegin={begin} onEnd={end} onChange={v=>patchFx(s.id,'threshold',+v)}/><Range label="Grain" min="0" max="100" value={f.grain} onBegin={begin} onEnd={end} onChange={v=>patchFx(s.id,'grain',+v)}/><Range label="Noise" min="0" max="100" value={f.noise} onBegin={begin} onEnd={end} onChange={v=>patchFx(s.id,'noise',+v)}/><Range label="Halftone" min="0" max="100" value={f.halftone} onBegin={begin} onEnd={end} onChange={v=>patchFx(s.id,'halftone',+v)}/><Range label="Dot size" min="2" max="40" value={f.halftoneSize||16} onBegin={begin} onEnd={end} onChange={v=>patchFx(s.id,'halftoneSize',+v)}/><Range label="Dot density" min="5" max="100" value={f.halftoneDensity||55} onBegin={begin} onEnd={end} onChange={v=>patchFx(s.id,'halftoneDensity',+v)}/><Range label="Dot opacity" min="0" max="100" value={f.halftoneOpacity||85} onBegin={begin} onEnd={end} onChange={v=>patchFx(s.id,'halftoneOpacity',+v)}/><Range label="Dot angle" min="-90" max="90" value={f.halftoneAngle||0} onBegin={begin} onEnd={end} onChange={v=>patchFx(s.id,'halftoneAngle',+v)}/><ColorControl label="Dot color" value={f.halftoneColor||'#000000'} onBegin={begin} onEnd={end} onChange={v=>patchFx(s.id,'halftoneColor',v)}/><Range label="Xerox" min="0" max="100" value={f.xerox} onBegin={begin} onEnd={end} onChange={v=>patchFx(s.id,'xerox',+v)}/><Range label="Fade" min="0" max="100" value={f.fade} onBegin={begin} onEnd={end} onChange={v=>patchFx(s.id,'fade',+v)}/></section>;
}
