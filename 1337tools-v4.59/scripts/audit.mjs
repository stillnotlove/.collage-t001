...
check(shell.includes('<span>4.59</span>'),'INDEX: version is not 4.59');
check((shell.match(/4\.59/g)||[]).length===1,'INDEX: version should appear only once in AppShell');
...
check(css.includes('--brand-digit-gap:2px'),'BRAND: equal 1337 digit gap variable missing');
check(css.includes('.credits337{'),'BRAND: credits 337 spacing rule missing');
check(css.includes('margin-left:var(--brand-digit-gap)!important'),'BRAND: equal digit gap override missing');
...
check(visualAudit.includes('<span>4.59</span>'),'QA: static visual audit has stale version');
...
check(pkg.version==='0.4.59','package.json: stale version');
