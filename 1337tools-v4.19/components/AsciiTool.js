...
export default function AsciiTool({onIndex,onSendToEditor,onContinue,trail=[],initialFile,initialRatio='4 / 5',initialToken}){
  ...
  async function loadFile(file){
    ...
    const img=new Image();
    img.onload=()=>{imgRef.current=img;setSource(url);setSourceName(file.name||'PASTED IMAGE');setSourceView(DEFAULT_VIEW);setSeed(Math.floor(Math.random()*1e9))};
    img.onerror=()=>{if(urlRef.current===url)urlRef.current=null;URL.revokeObjectURL(url);reportToolError('ASCII image load',new Error('Unsupported or damaged image'))};
    img.src=url;
  }
  ...
  return <main className="fieldShell asciiShell">
    <aside className="fieldHud">
      <div className="brand"><span className="brand1337">1337</span><span className="brandTools">tools</span><small>4.19 / 03 ASCII</small></div>
      ...
      <section><div className="title">INPUT</div><button className="wide" onClick={()=>fileRef.current?.click()}>+ SOURCE IMAGE</button><input ref={fileRef} type="file" hidden accept="image/*" onChange={e=>{loadFile(e.target.files?.[0]);e.target.value=''}}/><div className="note">{sourceName}<br/>drop anywhere / Ctrl/Cmd+V</div>
      ...
      <section><div className="title">OUTPUT</div><button className="wide primary" disabled={!source||exporting} onClick={sendToEditor}>SEND TO EDITOR</button><ContinuePanel current="ascii" disabled={!source||exporting} onChoose={continueTo}/><div className="two"><button disabled={!source||exporting} onClick={()=>exportPng(false)}>{exporting?'…':'PNG'}</button><button disabled={!source||exporting} onClick={()=>exportPng(true)}>{exporting?'…':'PNG α'}</button></div><div className="note">CONTINUE keeps the selected background. PNG α removes it.</div></section>
    </aside>
