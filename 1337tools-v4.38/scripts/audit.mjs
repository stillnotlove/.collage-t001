...
check(shell.includes('<span>4.38</span>'),'INDEX: version is not 4.38');
check((shell.match(/4\.38/g)||[]).length===1,'INDEX: version should appear only once in AppShell');
...
check(entry.includes('let renderedFx=false'),'ENTRY: effect snapshot cache missing');
check(entry.includes('function renderCurrentFx()'),'ENTRY: effect snapshot renderer missing');
check(entry.includes('ctx.clearRect(0,0,size.w,size.h)'),'ENTRY: frame-level canvas clear missing');
check(entry.includes("state='clean'"),'ENTRY: clean reset state missing');
...
check(css.includes('/* 4.38 — stability + detail cleanup */'),'VISUAL: 4.38 marker missing');
...
check(css.includes('box-shadow:6px 6px 0 rgba(255,255,255,.18)!important'),'INDEX: unified card underlay missing');
...
check(pkg.version==='0.4.38','package.json: stale version');
