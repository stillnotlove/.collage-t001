...
const DEFAULTS={mode:'ASCII',style:'RAW',density:88,scale:108,contrast:115,brightness:0,chaos:10,colorMode:'WHITE'};

export default function AsciiTool({onIndex,onSendToEditor,onContinue,trail=[],initialFile,initialRatio='4 / 5',initialToken}){
  ...
  function randomize(){
    ...
    setParams({
      ...
      scale:Math.round(74+Math.random()*80),
  ...
  function containRect(img,w,h){
    const iw=img?.naturalWidth||1,ih=img?.naturalHeight||1,scale=Math.min(w/iw,h/ih);
  ...
  function sampleImage(cols,rows){
    ...
    x.clearRect(0,0,cols,rows);drawContain(x,img,cols,rows);
  ...
  function render(ctx,w,h,{transparent=false}={}){
    ...
    if(!img){
      ...
      ctx.fillText('DROP / PASTE ONE IMAGE',w/2,h/2);
      return;
    }
    const rect=containRect(img,w,h),style=params.style||'RAW';
    const cols=Math.max(12,Math.round(params.density));
    const rows=Math.max(12,Math.round(cols*(rect.dh/Math.max(1,rect.dw))*(style==='STRAIGHT'?1.78:1.58)));
    const cellW=rect.dw/cols,cellH=rect.dh/rows,charString=(charset&&charset.length?charset:defaultCharset()),charSet=[...charString];
    const cells=sampleImage(cols,rows);if(!cells||!charSet.length)return;
    const rnd=mulberry32(hashSeed(`${seed}:${params.mode}:${params.style}:${charString}:${params.chaos}`));
    const chaos=style==='STRAIGHT'?Math.min(.18,(+params.chaos||0)/250):(+params.chaos||0)/100;
    const fontSize=Math.max(6,Math.min(cellW*(+params.scale||100)/100*(style==='STRAIGHT'?1.02:1.15),cellH*(style==='STRAIGHT'?1.06:1.22)));
    ...
    for(let y=0;y<rows;y++)for(let x=0;x<cols;x++){
      ...
      const lum=c.lum;let idx=Math.round(((255-lum)/255)*(charSet.length-1));
      if(chaos>0){idx+=Math.round((rnd()-.5)*2*Math.max(1,(charSet.length-1)*(style==='STRAIGHT'?.05:.12)*chaos))}
      idx=Math.max(0,Math.min(charSet.length-1,idx));
      const ch=charSet[idx]||' ';if(ch===' ')continue;
      const jx=style==='STRAIGHT'?(rnd()-.5)*cellW*.08*chaos:(rnd()-.5)*cellW*.34*chaos;
      const jy=style==='STRAIGHT'?(rnd()-.5)*cellH*.06*chaos:(rnd()-.5)*cellH*.34*chaos;
      const rot=style==='STRAIGHT'?(rnd()-.5)*0.02:(rnd()-.5)*0.28*chaos;
      ctx.save();
      ctx.globalAlpha=clamp(c.a,0,1);
      ctx.fillStyle=params.colorMode==='SOURCE'?`rgb(${Math.round(c.r)},${Math.round(c.g)},${Math.round(c.b)})`:params.colorMode==='YELLOW'?'#ffd800':'#1738d9';
      if(transparent&&params.colorMode!=='SOURCE'&&params.colorMode!=='YELLOW')ctx.fillStyle='#1738d9';
      if(!transparent&&params.colorMode==='WHITE')ctx.fillStyle='#1738d9';
      ctx.translate(rect.dx+(x+.5)*cellW+jx,rect.dy+(y+.54)*cellH+jy);ctx.rotate(rot);ctx.fillText(ch,0,0);ctx.restore();
    }
  }
  ...
  async function sendToEditor(){
    ...
    try{const b=await renderBlob({transparent:true}),f=new File([b],`ASCII-${Date.now()}.png`,{type:'image/png'});onSendToEditor?.(f,ratio)}
    catch(err){reportToolError('ASCII → EDITOR',err)}finally{setExporting(false)}
  }
  async function continueTo(target){
    if(!imgRef.current||exporting)return;setExporting(true);
    try{const b=await renderBlob({transparent:true}),f=new File([b],`ASCII-${target}-${Date.now()}.png`,{type:'image/png'});onContinue?.('ascii',target,f,ratio)}
  ...
  return <main className="fieldShell asciiShell">
    <aside className="fieldHud">
      <div className="brand"><span className="brand1337">1337</span><span className="brandTools">tools</span><small>4.15 / 03 ASCII</small></div>
      ...
      <section><div className="title">ASCII</div>
        ...
        <AsciiSelect label="Style" value={params.style} options={STYLES} onChange={v=>{setP('style',v);setCharset(defaultCharset(v,params.mode));if(v==='STRAIGHT'&&params.chaos>24)setP('chaos',24)}}/>
      ...
      <section><div className="title">OUTPUT</div><button className="wide primary" disabled={!source||exporting} onClick={sendToEditor}>SEND TO EDITOR</button><ContinuePanel current="ascii" disabled={!source||exporting} onChoose={continueTo}/><div className="two"><button disabled={!source||exporting} onClick={()=>exportPng(false)}>{exporting?'…':'PNG'}</button><button disabled={!source||exporting} onClick={()=>exportPng(true)}>{exporting?'…':'PNG α'}</button></div><div className="note">Style = RAW / STRAIGHT. PNG α = export without background.</div></section>
    </aside>
