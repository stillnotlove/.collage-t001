...
export default function EntryLiveMark(){
  ...
  useEffect(()=>{
    ...
    function overlapIntoAscii(ctx,fromFrame,asciiFrame,p){
      // Keep the outgoing motion visible while the glyph field is already forming on top.
      const asciiIn=smootherstep(clamp01((p-.04)/.72));
      const motionOut=1-smootherstep(clamp01((p-.48)/.52));
      ctx.globalAlpha=motionOut;ctx.drawImage(fromFrame,0,0);
      ctx.globalAlpha=asciiIn;ctx.drawImage(asciiFrame,0,0);
      ctx.globalAlpha=1;
    }

    function overlapOutOfAscii(ctx,asciiFrame,motionFrame,p){
      // The next physical motion starts before the ASCII texture has fully disappeared.
      const motionIn=smootherstep(clamp01((p-.02)/.74));
      const asciiOut=1-smootherstep(clamp01((p-.44)/.56));
      ctx.globalAlpha=motionIn;ctx.drawImage(motionFrame,0,0);
      ctx.globalAlpha=asciiOut;ctx.drawImage(asciiFrame,0,0);
      ctx.globalAlpha=1;
    }
    ...
    function frame(now){
      ...
      if(phase==='hold')renderNode(nodes[nodeIndex],canvas,timeSec,1);
        ...
        const fromIntensity=fromIsMotion?(intoAscii?(.78+.22*(1-p)):(.42+.58*(1-p))):1;
        const toIntensity=toIsMotion?(outOfAscii?(.35+.65*p):(.38+.62*p)):1;
        renderNode(from,frameA,timeSec,fromIntensity);
        renderNode(to,frameB,timeSec,toIntensity);

        if(intoAscii)overlapIntoAscii(ctx,frameA,frameB,p);
        else if(outOfAscii)overlapOutOfAscii(ctx,frameA,frameB,p);
