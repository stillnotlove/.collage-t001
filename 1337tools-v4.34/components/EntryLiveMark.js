...
const ROUTES=[
  ['scan','outline','asciiSquare','rebuildSoft'],
  ['slice','asciiBlock','rebuildSlice'],
  ['columns','asciiSquare','ascii','rebuildColumns'],
  ['echo','tiles','asciiBraille','rebuildSoft'],
  ['distress','asciiSquare','asciiBraille','rebuildGlyph'],
  ['pixel','scan','asciiBlock','rebuildColumns'],
  ['ascii','distress','outline','rebuildSoft'],
  ['slice','columns','asciiBraille','rebuildSlice'],
  ['asciiBraille','echo','asciiBlock','rebuildGlyph'],
  ['columns','pixel','asciiSquare','rebuildColumns'],
  ['asciiBlock','scan','distress','rebuildSoft'],
  ['echo','slice','asciiBraille','rebuildSlice'],
  ['outline','asciiSquare','tiles','rebuildSoft'],
  ['tiles','asciiBraille','echo','rebuildGlyph'],
  ['outline','echo','asciiBlock','rebuildColumns'],
  ['scan','tiles','distress','asciiSquare','rebuildSlice'],
  ['pixel','outline','rebuildGlyph'],
  ['columns','echo','scan','rebuildColumns'],
];
const EFFECTS=['slice','ascii','asciiSquare','asciiBraille','asciiBlock','distress','echo','pixel','columns','outline','tiles','scan'];
...
export default function EntryLiveMark(){
  ...
  useEffect(()=>{
    ...
    let base=makeCanvas(1,1),settled=makeCanvas(1,1),workA=makeCanvas(1,1),workB=makeCanvas(1,1),sample=makeCanvas(1,1);
    ...
    function resize(){
      ...
      size={w,h,dpr};canvas.width=w;canvas.height=h;base=makeCanvas(w,h);settled=makeCanvas(w,h);workA=makeCanvas(w,h);workB=makeCanvas(w,h);sample=makeCanvas(1,1);drawBase();copy(settled,base);return true;
    }
    ...
    function renderAscii(target,source,seed,time,amount,forcedVariant=null){
      ...
      ctx.globalAlpha=.28+(1-amount)*.72;ctx.drawImage(source,0,0);ctx.globalAlpha=amount;
      const charsets={
        glyph:[...'@#%*+=-:.'],
        square:[...'■▪▫·'],
        block:[...'█▓▒░'],
    ...
    function renderOutline(target,source,seed,time,amount){
      const {w,h,dpr}=size,ctx=target.getContext('2d');ctx.clearRect(0,0,w,h);if(amount<=.001){ctx.drawImage(source,0,0);return}
      const rnd=mulberry32(hashSeed(`${seed}:outline`)),r=Math.max(1,Math.round((1.2+rnd()*2.4)*dpr*amount)),steps=8;
      ctx.globalAlpha=.15+(1-amount)*.85;ctx.drawImage(source,0,0);ctx.globalAlpha=.45+.35*amount;
      for(let i=0;i<steps;i++){const a=Math.PI*2*i/steps,dx=Math.cos(a)*r,dy=Math.sin(a)*r;ctx.drawImage(source,dx,dy)}
      ctx.globalCompositeOperation='destination-out';ctx.globalAlpha=.58+.34*amount;ctx.drawImage(source,0,0);ctx.globalCompositeOperation='source-over';
      ctx.globalAlpha=.14+.2*(1-amount);ctx.drawImage(source,0,0);ctx.globalAlpha=1;
    }

    function renderTiles(target,source,seed,time,amount){
      const {w,h}=size,ctx=target.getContext('2d');ctx.clearRect(0,0,w,h);if(amount<=.001){ctx.drawImage(source,0,0);return}
      const rnd=mulberry32(hashSeed(`${seed}:tiles`)),cols=10+Math.floor(rnd()*12),rows=4+Math.floor(rnd()*7),cw=w/cols,ch=h/rows,phase=time*.0011;
      ctx.globalAlpha=.2*(1-amount);ctx.drawImage(source,0,0);ctx.globalAlpha=1;
      for(let gy=0;gy<rows;gy++)for(let gx=0;gx<cols;gx++){
        const sx=gx*cw,sy=gy*ch,local=mulberry32(hashSeed(`${seed}:${gx}:${gy}`)),ampX=w*(.002+.01*local())*amount,ampY=h*(.002+.014*local())*amount;
        const dx=Math.sin(gx*.83+gy*.47+phase)*ampX,dy=Math.cos(gx*.39-gy*.72+phase*.8)*ampY;
        ctx.globalAlpha=.72+.28*local();ctx.drawImage(source,sx,sy,cw+.5,ch+.5,sx+dx,sy+dy,cw+.5,ch+.5);
      }
      ctx.globalAlpha=1;
    }
    ...
    function applyEffect(target,source,name,seed,time,amount){
      ...
      else if(name==='outline')renderOutline(target,source,seed,time,amount);
      else if(name==='tiles')renderTiles(target,source,seed,time,amount);
    ...
    function beginCycle(now){
      route=generateRoute();const rnd=mulberry32(hashSeed(`${sessionSeed}:${cycle}:timing`));seeds=route.map((_,i)=>Math.floor(rnd()*1e9)+i);copy(settled,base);stage=0;stageStart=now;stageDuration=1050+rnd()*1150;
    }
    function advance(now){
      stage++;
      if(stage>=route.length){cycle++;stage=-1;const rnd=mulberry32(hashSeed(`${sessionSeed}:${cycle}:pause`));pauseUntil=now+3400+rnd()*3600;return}
      const rnd=mulberry32(hashSeed(`${sessionSeed}:${cycle}:${stage}:duration`));stageStart=now;const rebuilding=route[stage]?.startsWith('rebuild');stageDuration=(rebuilding?3200:820)+rnd()*(rebuilding?2600:1900);
    }

    function commitStage(now){
      if(stage<0||!route[stage])return;
      if(route[stage].startsWith('rebuild'))renderRebuild(workA,settled,seeds[stage],now,1,route[stage]);
      else applyEffect(workA,settled,route[stage],seeds[stage],now,1);
      copy(settled,workA);
    }

    function renderPipeline(now,p){
      if(route[stage]?.startsWith('rebuild'))renderRebuild(workB,settled,seeds[stage],now,p,route[stage]);
      else applyEffect(workB,settled,route[stage],seeds[stage],now,p);
      return workB;
    }

    function frame(now){
      if(disposed)return;const resized=resize();if(resized&&stage>=0){cycle++;stage=-1;pauseUntil=now+520}const ctx=canvas.getContext('2d');ctx.clearRect(0,0,size.w,size.h);
      ...
      if(raw>=1){commitStage(now);advance(now);raw=0;if(stage<0){ctx.drawImage(base,0,0);raf=requestAnimationFrame(frame);return}}
      const p=smoothstep(raw),output=renderPipeline(now,p);ctx.drawImage(output,0,0);
      if(!route[stage]?.startsWith('rebuild')){ctx.save();ctx.globalAlpha=.085+.065*(1-p);ctx.drawImage(base,0,0);ctx.restore()}
