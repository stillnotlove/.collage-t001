...
export default function EntryLiveMark(){
  ...
  useEffect(()=>{
    ...
    function morphMotionIntoAscii(ctx,from,asciiFrame,p,timeSec){
      // True source morph: the moving SLICE/ECHO itself becomes ASCII before the motion settles.
      // This avoids a visible 'old effect off -> ASCII on' hand-off.
      const sourceP=smootherstep(clamp01((p-.03)/.84));
      const sourceCtx=frameC.getContext('2d');
      sourceCtx.clearRect(0,0,size.w,size.h);sourceCtx.globalCompositeOperation='source-over';sourceCtx.imageSmoothingEnabled=true;
      sourceCtx.globalAlpha=1-sourceP;sourceCtx.drawImage(base,0,0);
      sourceCtx.globalAlpha=sourceP;sourceCtx.drawImage(asciiFrame,0,0);sourceCtx.globalAlpha=1;

      // Keep most of the physical motion while glyphs are already replacing the clean wordmark,
      // then let that same motion continuously decay into the final static ASCII frame.
      const settle=smootherstep(clamp01((p-.42)/.58));
      const intensity=1-settle;
      if(from.kind==='slice')renderSliceSource(frameA,frameC,from,timeSec,intensity);
      else renderEchoSource(frameA,frameC,from,timeSec,intensity);

      ctx.globalAlpha=1;ctx.drawImage(frameA,0,0);
      // Final microscopic settle removes slice band seams / echo residue without creating a switch.
      const finalLock=smootherstep(clamp01((p-.86)/.14));
      if(finalLock>0){ctx.globalAlpha=finalLock;ctx.drawImage(asciiFrame,0,0)}
      ctx.globalAlpha=1;
    }
