...
check(shell.includes('<span>4.50</span>'),'INDEX: version is not 4.50');
check((shell.match(/4\.50/g)||[]).length===1,'INDEX: version should appear only once in AppShell');
...
check(entry.includes('morphMotionIntoAscii'),'ENTRY: moving effect → ASCII source-morph compositor missing');
...
check(visualAudit.includes('<span>4.50</span>'),'QA: static visual audit has stale version');
...
check(pkg.version==='0.4.50','package.json: stale version');
