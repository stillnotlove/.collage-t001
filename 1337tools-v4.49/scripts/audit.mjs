...
check(shell.includes('<span>4.49</span>'),'INDEX: version is not 4.49');
check((shell.match(/4\.49/g)||[]).length===1,'INDEX: version should appear only once in AppShell');
...
check(entry.includes('overlapIntoAscii'),'ENTRY: moving effect → ASCII overlap compositor missing');
...
check(visualAudit.includes('<span>4.49</span>'),'QA: static visual audit has stale version');
...
check(pkg.version==='0.4.49','package.json: stale version');
