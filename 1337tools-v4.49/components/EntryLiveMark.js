...
export default function EntryLiveMark(){
  ...
  useEffect(()=>{
    ...
    function overlapIntoAscii(ctx,fromFrame,asciiFrame,p,seed){
      // Motion keeps travelling while ASCII progressively colonizes the same wordform.
      const motionOut=1-smootherstep(clamp01((p-.60)/.40));
      ctx.globalAlpha=.36+.64*motionOut;ctx.drawImage(fromFrame,0,0);
      drawAsciiReveal(ctx,asciiFrame,clamp01((p-.02)/.88),seed,false);
      ctx.globalAlpha=1;
    }
    ...
    function frame(now){
      ...
      if(phase==='hold')renderNode(nodes[nodeIndex],canvas,timeSec,1);
        ...
        if(intoAscii)overlapIntoAscii(ctx,frameA,frameB,p,transitionSeed);
