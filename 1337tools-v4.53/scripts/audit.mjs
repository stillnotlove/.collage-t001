...
check(shell.includes('<span>4.53</span>'),'INDEX: version is not 4.53');
check((shell.match(/4\.53/g)||[]).length===1,'INDEX: version should appear only once in AppShell');
...
check(entry.includes('const ROUTE_TEMPLATES=['),'ENTRY: randomized route templates missing');
check(entry.includes("nodes=[{kind:'clean'},...route,{kind:'clean'}]"),'ENTRY: clean → 3 effects → clean sequence missing');
check(entry.includes("['ascii','slice','ascii']"),'ENTRY: preferred ASCII → SLICE → ASCII route missing');
check(entry.includes('renderSlice(target,desc,timeSec,intensity=1)'),'ENTRY: dynamic SLICE renderer missing');
check(entry.includes('const pulse=.32+.68*(.5-.5*Math.cos'),'ENTRY: SLICE out/in pulse missing');
check(entry.includes('renderEcho(target,desc,timeSec,intensity=1)'),'ENTRY: lightweight randomized echo effect missing');
check(entry.includes('route=template.map'),'ENTRY: three-effect route construction missing');
check(entry.includes('morphMotionIntoAscii'),'ENTRY: moving effect → ASCII morph router missing');
check(entry.includes('renderSliceIntoAscii'),'ENTRY: dedicated SLICE → ASCII band morph missing');
check(entry.includes('renderEchoIntoAscii'),'ENTRY: dedicated ECHO → ASCII copy morph missing');
check(entry.includes('overlapOutOfAscii'),'ENTRY: ASCII → moving effect overlap compositor missing');

check(entry.includes('drawAsciiReveal'),'ENTRY: progressive ASCII reveal compositor missing');
check(entry.includes('overlapMotionToMotion'),'ENTRY: motion-to-motion overlap compositor missing');
check(entry.includes('transitionSeed'),'ENTRY: deterministic transition reveal seed missing');
check(entry.includes("['echo','ascii','slice']")&&entry.includes("['ascii','echo','ascii']"),'ENTRY: ECHO → ASCII live routes missing');
check(entry.includes("const intoAscii=fromIsMotion&&to.kind===\'ascii\'"),'ENTRY: live motion-to-ASCII transition guard missing');
check(entry.includes('usedAscii=new Set()'),'ENTRY: repeated ASCII variant guard missing');
...
check(!entry.includes("'pixel'")&&!entry.includes("'columns'")&&!entry.includes("'distress'"),'ENTRY: heavy/non-requested effect returned');
check(entry.includes('smootherstep'),'ENTRY: high-quality easing missing');
check(entry.includes('resizeDirty'),'ENTRY: resize dirty-flag optimization missing');
check(entry.includes('visibilitychange'),'ENTRY: visibility pause/resume guard missing');
check(entry.includes('while((now-phaseStart)>=phaseDuration'),'ENTRY: phase catch-up guard missing');
...
check(visualAudit.includes('<span>4.53</span>'),'QA: static visual audit has stale version');
...
check(pkg.version==='0.4.53','package.json: stale version');
