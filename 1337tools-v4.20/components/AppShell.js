...
const MODES=[
  {id:'field',n:'01',name:'FIELD',alias:'DISTRIBUTE'},
  {id:'slice',n:'02',name:'SLICE',alias:'REORDER'},
  {id:'ascii',n:'03',name:'ASCII',alias:'GLYPH'},
  {id:'editor',n:'04',name:'EDITOR',alias:'COMPOSE'},
];
...
export default function AppShell(){
  ...
  return <CollageEditor initialFile={editorFile} initialRatio={editorRatio} trail={trail} onIndex={()=>{resetEditor();setTrail([]);setScreen('index')}}/>;
}
...
function Brand(){return <LogoWord/>}
...
function IndexScreen({onOpen}){
  ...
  return <main className="indexScreen indexMinimal" data-active={active.id}>
    <header className="minimalHeader"><Brand/><span>4.20</span></header>
    <nav className="minimalModes" aria-label="1337tools">
      {MODES.map(m=><button key={m.id} type="button" className={`minimalMode ${m.id===hovered?'isHovered':''} isActive`} onMouseEnter={()=>setHovered(m.id)} onFocus={()=>setHovered(m.id)} onClick={()=>onOpen(m.id)}>
        <span>{m.n}</span><strong className="modeLabelWrap"><i>{m.name}</i><em>{m.alias}</em></strong><b>↗</b>
      </button>)}
    </nav>
