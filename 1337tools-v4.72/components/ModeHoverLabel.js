...
export default function ModeHoverLabel({mode,label,active}){
  ...
  useEffect(()=>{
    ...
    let raf=0,ro=null,start=performance.now(),disposed=false,resizeDirty=true;
    let source=makeCanvas(1,1),mask=makeCanvas(1,1),asciiSample=makeCanvas(1,1),ditherSample=makeCanvas(1,1),size={w:1,h:1,dpr:1};
    ...
    function drawDither(ctx,p,time){
      const {w,h,dpr}=size,rnd=mulberry32(hashSeed(`${seedRef.current}:${mode}:dither`)),cell=Math.max(3,Math.round((5+rnd()*5)*dpr));
      const sw=Math.max(1,Math.ceil(w/cell)),sh=Math.max(1,Math.ceil(h/cell));
      if(ditherSample.width!==sw||ditherSample.height!==sh){ditherSample.width=sw;ditherSample.height=sh}
      const sx=ditherSample.getContext('2d',{willReadFrequently:true});sx.clearRect(0,0,sw,sh);sx.drawImage(mask,0,0,sw,sh);
      const px=sx.getImageData(0,0,sw,sh).data,bayer=[0,8,2,10,12,4,14,6,3,11,1,9,15,7,13,5];
      ctx.globalAlpha=1-p;ctx.drawImage(source,0,0);ctx.globalAlpha=p;ctx.fillStyle='#ffffff';
      for(let y=0;y<sh;y++)for(let x=0;x<sw;x++){
        const q=(y*sw+x)*4,a=px[q+3]/255;if(a<.08)continue;
        const threshold=(bayer[(y%4)*4+(x%4)]+.5)/16;
        if(a>threshold*.78){const j=(rnd()-.5)*cell*.12*p;ctx.fillRect(x*cell+j,y*cell,Math.max(1,cell*.78),Math.max(1,cell*.78))}
      }
      ctx.globalAlpha=1;
    }
    ...
    function frame(now){
      if(disposed)return;if(resizeDirty){resizeDirty=false;resize()}const ctx=canvas.getContext('2d');ctx.clearRect(0,0,size.w,size.h);
      const elapsed=now-start,p=ease(Math.min(1,elapsed/320));
      if(mode==='ascii')drawAscii(ctx,p,now);else if(mode==='slice')drawSlice(ctx,p);else if(mode==='editor')drawEditor(ctx,p,now);else if(mode==='echo')drawEcho(ctx,p,now);else if(mode==='dither')drawDither(ctx,p,now);else drawField(ctx,p,now);
    ...
    if(typeof ResizeObserver!=='undefined'){ro=new ResizeObserver(()=>{resizeDirty=true});ro.observe(wrap)}
