...
const ctxOf=c=>{const x=c.getContext('2d',{alpha:true,willReadFrequently:false});if(x){x.imageSmoothingEnabled=true;try{x.imageSmoothingQuality='high'}catch{}}return x};
...
async function renderImageLayer(l,w,h,imageCache,renderScale=1){
  ...
  b.filter=`brightness(${1+f.exposure/100}) contrast(${f.contrast/100+th*5+f.xerox/80}) saturate(${f.saturation/100}) grayscale(${Math.max(f.grayscale/100,th,f.xerox/120)}) blur(${Math.max(0,+f.blur||0)*renderScale}px)`;
...
function layerRasterSize(w,h){return Math.max(1000,Math.min(4096,Math.ceil(Math.max(w||0,h||0))))}
function renderShapeLayer(l,w=1000,h=1000){
  const res=layerRasterSize(w,h),c=canvasOf(res,res),x=ctxOf(c),unit=res/1000;
  x.scale(unit,unit);
  const sw=Math.max(0,+l.strokeWidth||0)*10,fill=l.fill||'#111',stroke=l.stroke||fill;
...
function textPass(l,{fill,stroke,strokeWidth=0,mode='fill',resolution=1000}={}){
  const c=canvasOf(resolution,resolution),x=ctxOf(c),unit=resolution/1000;x.save();x.scale(unit,unit);x.translate(500,500);x.scale(+l.scaleX||1,+l.scaleY||1);x.translate(-500,-500);x.fillStyle=fill||l.color||'#111';x.strokeStyle=stroke||l.outlineColor||'#111';x.lineWidth=strokeWidth;x.lineJoin='round';paintTextLines(x,l,mode);x.restore();return c;
}
function renderTextLayer(l,w=1000,h=1000){
  const res=layerRasterSize(w,h),out=canvasOf(res,res),x=ctxOf(out),ow=Math.max(0,+l.outline||0)*6.5,mode=l.outlineMode||'center';
  const fill=textPass(l,{fill:l.color,mode:'fill',resolution:res});
  if(ow<=0){x.drawImage(fill,0,0);return out}
  const stroke=textPass(l,{stroke:l.outlineColor,strokeWidth:mode==='center'?ow:ow*2,mode:'stroke',resolution:res});
  if(mode==='inside'){
...
function renderKey(l,w,h,renderScale=1){
  const common={type:l.type,width:w,height:h,renderScale};
...
async function renderLayer(l,docWidth,docHeight,imageCache,renderCache,renderScale=1){
  const w=Math.max(1,Math.round(docWidth*(+l.width||1)/100)),h=Math.max(1,Math.round(docHeight*(+l.height||1)/100)),key=renderKey(l,w,h,renderScale);
  if(renderCache.has(key))return renderCache.get(key);
  const task=(async()=>{
    if(l.type==='image')return {canvas:await renderImageLayer(l,w,h,imageCache,renderScale),w,h};
    if(l.type==='text')return {canvas:renderTextLayer(l,w,h),w,h};
    return {canvas:renderShapeLayer(l,w,h),w,h};
  })();
...
export async function exportScene(layers,{kind='png',background='#fff',transparent=false,docWidth=1200,docHeight=1500,scale=1}={}){
  if(!Array.isArray(layers))throw new Error('Scene layers are missing');
  const renderScale=clamp(Number.isFinite(+scale)?+scale:1,1,4),outWidth=Math.max(1,Math.round(docWidth*renderScale)),outHeight=Math.max(1,Math.round(docHeight*renderScale));
  const visible=[...layers].filter(l=>!l.hidden).sort((a,b)=>(a.order||0)-(b.order||0));
  await ensureFonts(visible);
  const out=canvasOf(outWidth,outHeight),ctx=ctxOf(out);if(!transparent){ctx.fillStyle=background;ctx.fillRect(0,0,outWidth,outHeight);}
  const imageCache=new Map(),renderCache=new Map();
  for(const l of visible){
    try{
      const {canvas,w,h}=await renderLayer(l,outWidth,outHeight,imageCache,renderCache,renderScale),cx=outWidth*(+l.x||0)/100,cy=outHeight*(+l.y||0)/100;
