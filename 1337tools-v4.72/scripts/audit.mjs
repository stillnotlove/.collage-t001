...
const tools={
  ...
  DITHER:'components/DitherTool.js',
};
...
check(shell.includes('<span>4.72</span>'),'INDEX: version is not 4.72');
check((shell.match(/4\.72/g)||[]).length===1,'INDEX: version should appear only once in AppShell');
...
check(!css.includes('content:"EDITOR"')&&!css.includes('content:"FIELD"')&&!css.includes('content:"SLICE"')&&!css.includes('content:"ASCII"')&&!css.includes('content:"ECHO"')&&!css.includes('content:"DITHER"'),'WORKSPACE: stale giant tool labels remain in CSS');
...
for(const file of ['components/SliceTool.js','components/AsciiTool.js','components/EchoTool.js','components/FieldTool.js','components/DitherTool.js']){
  const txt=read(file);
...
check(visualAudit.includes('<span>4.72</span>'),'QA: static visual audit has stale version');
...
// HQ PNG export patch (kept inside v4.72)
...
check(pkg.version==='0.4.72','package.json: stale version');
...
check(shell.includes("id:'dither',n:'06',name:'DITHER'"),'DITHER: catalog mode missing');
check(shell.includes("screen==='dither'"),'DITHER: AppShell route missing');
check(shell.includes("dither:'DITHER'"),'DITHER: analytics/tool-name mapping missing');
const continuePanel=read('components/ContinuePanel.js');
check(continuePanel.includes("id:'dither',label:'TO DITHER'"),'DITHER: CONTINUE option missing');
const dither=read('components/DitherTool.js');
for(const mode of ['THRESHOLD','BAYER 4×4','BAYER 8×8','FLOYD','ATKINSON','NOISE'])check(dither.includes(mode),`DITHER: mode missing ${mode}`);
check(dither.includes('RANDOMIZE DITHER'),'DITHER: randomize action missing');
check(dither.includes("onContinue?.('dither',target,f,ratio)"),'DITHER: continue route missing');
check(dither.includes('SEND TO EDITOR'),'DITHER: editor handoff missing');
check(dither.includes('loadSeq'),'DITHER: image load race guard missing');
check(dither.includes('PNG α'),'DITHER: alpha export missing');
check(css.includes('data-mode="dither"'),'DITHER: catalog styles missing');
check(hover.includes("mode==='dither'"),'DITHER: hover visual missing');
check(visualAudit.includes('06 DITHER'),'QA: static visual audit missing DITHER marker');

// 4.72 cross-tool regression matrix
const routeIds=['field','slice','ascii','echo','dither'];
const routeFiles={field:'components/FieldTool.js',slice:'components/SliceTool.js',ascii:'components/AsciiTool.js',echo:'components/EchoTool.js',dither:'components/DitherTool.js'};
for(const id of routeIds){
  const txt=read(routeFiles[id]);
  check(txt.includes(`current="${id}"`),`${id.toUpperCase()}: CONTINUE current id mismatch`);
  check(txt.includes(`onContinue?.('${id}',target`),`${id.toUpperCase()}: tool-to-tool handoff id mismatch`);
  check(txt.includes('renderBlob'),`${id.toUpperCase()}: renderBlob export contract missing`);
  check(txt.includes('downloadBlob'),`${id.toUpperCase()}: download export contract missing`);
  check(txt.includes('onSendToEditor'),`${id.toUpperCase()}: editor handoff prop missing`);
  check(txt.includes('RANDOMIZE'),`${id.toUpperCase()}: random generator action missing`);
}
const optionIds=[...continuePanel.matchAll(/\{id:'([^']+)',label:'TO [^']+'\}/g)].map(m=>m[1]);
check(optionIds.length===6,'CONTINUE: expected exactly 6 destinations including EDITOR');
check(new Set(optionIds).size===optionIds.length,'CONTINUE: duplicate destination ids');
for(const id of ['field','slice','ascii','editor','echo','dither'])check(optionIds.includes(id),`CONTINUE: destination missing ${id}`);
for(const [id,name] of Object.entries({field:'FIELD',slice:'SLICE',ascii:'ASCII',editor:'EDITOR',echo:'ECHO',dither:'DITHER'})){
  check(shell.includes(`id:'${id}'`)||id==='editor',`INDEX: missing ${name} catalog id`);
  check(shell.includes(`${id}:'${name}'`),`ANALYTICS: missing ${name} TOOL_NAMES mapping`);
}
check(hover.includes('ditherSample'),'DITHER hover: reusable sample canvas missing');
