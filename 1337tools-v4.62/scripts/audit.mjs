...
check(shell.includes('<span>4.62</span>'),'INDEX: version is not 4.62');
check((shell.match(/4\.62/g)||[]).length===1,'INDEX: version should appear only once in AppShell');
...
check(css.includes('--brand-main-digit-kern:-0.078em'),'BRAND: main wordmark digit kern variable missing');
check(css.includes('--credits-digit-kern:-0.052em'),'BRAND: credits wordmark digit kern variable missing');
check(css.includes('.credits337{'),'BRAND: credits 337 spacing rule missing');
check(css.includes('margin-left:var(--brand-main-digit-kern)!important'),'BRAND: main tight digit kern override missing');
check(css.includes('margin-left:var(--credits-digit-kern)!important'),'BRAND: credits tight digit kern override missing');
...
check(visualAudit.includes('<span>4.62</span>'),'QA: static visual audit has stale version');
...
check(pkg.version==='0.4.62','package.json: stale version');
