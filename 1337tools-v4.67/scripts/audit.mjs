...
check(shell.includes('<span>4.67</span>'),'INDEX: version is not 4.67');
check((shell.match(/4\.67/g)||[]).length===1,'INDEX: version should appear only once in AppShell');
...
check(visualAudit.includes('<span>4.67</span>'),'QA: static visual audit has stale version');
...
for(const icon of ['app/favicon.ico','app/icon.png','app/apple-icon.png','public/favicon.ico','public/icon-48.png','public/icon-192.png','public/icon-512.png','public/apple-icon.png','public/safari-pinned-tab.svg'])check(exists(icon),`ICONS: missing ${icon}`);
check(layout.includes("'/favicon.ico'"),'ICONS: stable favicon metadata missing');
check(layout.includes("'/icon-48.png'"),'ICONS: 48px browser icon missing from metadata');
...
check(manifest.includes("'/icon-48.png'"),'ICONS: 48px manifest icon missing');
...
check(layout.includes('<link rel="apple-touch-icon" sizes="180x180" href="/apple-icon.png"'),'ICONS: explicit Apple touch icon missing');

const pkg=JSON.parse(read('package.json'));
check(pkg.version==='0.4.67','package.json: stale version');
