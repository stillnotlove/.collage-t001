...
check(shell.includes('<span>4.39</span>'),'INDEX: version is not 4.39');
check((shell.match(/4\.39/g)||[]).length===1,'INDEX: version should appear only once in AppShell');
...
for(const fx of ['asciiSquare','asciiBraille','asciiBlock','slice','echo','pixel','columns','distress'])check(entry.includes(fx),`ENTRY: ${fx} effect missing`);
check(!entry.includes('renderOutline'),'ENTRY: heavy outline renderer should be removed');
check(!entry.includes('renderTiles'),'ENTRY: heavy tiles renderer should be removed');
check(!entry.includes("'outline'"),'ENTRY: outline remains in the active effect grammar');
check(!entry.includes("'tiles'"),'ENTRY: tiles remains in the active effect grammar');
check(entry.includes('const ROUTES=['),'ENTRY: multi-step route templates missing');
check(entry.includes('let routeFrames=[]'),'ENTRY: pre-rendered route frame cache missing');
check(entry.includes("phase='transitionBetween'"),'ENTRY: effect-to-effect transition missing');
check(entry.includes('function buildRoute()'),'ENTRY: route pre-render function missing');
...
check(css.includes('/* 4.39 — multi-step entry routes / hover-only card underlay */'),'VISUAL: 4.39 marker missing');
...
check(entry.includes("const EFFECTS=['slice','ascii','asciiSquare','asciiBraille','asciiBlock','echo','pixel','columns','distress']"),'ENTRY: safe effect grammar changed unexpectedly');
...
check(pkg.version==='0.4.39','package.json: stale version');
