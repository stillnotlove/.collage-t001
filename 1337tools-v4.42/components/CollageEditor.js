...
export default function CollageEditor({initialFile=null,initialRatio='4 / 5',trail=[],onIndex,onHome}){
  ...
  async function addFiles(list){
    ...
    for(const f of files){
      const src=ownUrl(f);
      try{
        const d=await imgSize(src),ar=d.width/d.height,canvasAspect=doc.width/doc.height;
        ...
        adds.push({id:uid(),type:'image',name:f.name,src,originalSrc:src,local:true,x:50,y:50,width,height,rotation:0,opacity:1,blend:'normal',hidden:false,locked:false,order:order++,fit:'cover',radius:0,fx:{...fx0},fxSeed:Math.floor(Math.random()*1e9)});
