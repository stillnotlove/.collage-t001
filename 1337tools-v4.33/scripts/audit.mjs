...
check(shell.includes('<span>4.33</span>'),'INDEX: version is not 4.33');
check((shell.match(/4\.33/g)||[]).length===1,'INDEX: version should appear only once in AppShell');
...
check(shell.includes('1337tools <span>by</span>'),'CREDITS: title missing');
...
check(css.includes('/* 4.33 — controlled excess / editorial-chaos system */'),'VISUAL: 4.33 visual-system marker missing');
...
check(css.includes('border-radius:50%'),'HUD: art slider alternate thumb language missing');
...
check(css.includes('/* sliders: expressive marks, no ruler/ticks */'),'HUD: expressive slider system missing');
check(entry.includes("'scan'"),'ENTRY: scan effect missing from route grammar');

const pkg=JSON.parse(read('package.json'));
check(pkg.version==='0.4.33','package.json: stale version');
