...
export default function EchoTool({onIndex,onHome,onSendToEditor,onContinue,trail=[],initialFile,initialRatio='4 / 5',initialToken}){
  ...
  const canvasRef=useRef(null),fileRef=useRef(null),imgRef=useRef(null),urlRef=useRef(null),originalFileRef=useRef(null);
  ...
  async function loadFile(file,{preserveOriginal=false,isCutout=false}={}){
    ...
    if(!preserveOriginal)originalFileRef.current=file;
    if(urlRef.current)URL.revokeObjectURL(urlRef.current);
    const url=URL.createObjectURL(file);urlRef.current=url;
    const img=new Image();
    return await new Promise(resolve=>{
      img.onload=()=>{imgRef.current=img;setSource(url);setSourceName(file.name||'PASTED IMAGE');setSeed(Math.floor(Math.random()*1e9));setCutout(!!isCutout);resolve(true)};
      img.onerror=()=>{if(urlRef.current===url)urlRef.current=null;URL.revokeObjectURL(url);reportToolError('ECHO image load',new Error('Unsupported or damaged image'));resolve(false)};
      img.src=url;
    });
  }
  ...
  async function removeBg(){
    ...
    setCutting(true);
    try{
      const blob=await experimentalCutout(source,Math.floor(Math.random()*1e9));
      const file=new File([blob],`ECHO-CUTOUT-${Date.now()}.png`,{type:'image/png'});
      await loadFile(file,{preserveOriginal:true,isCutout:true});
    }catch(err){reportToolError('ECHO remove BG',err)}finally{setCutting(false)}
  }
  ...
  return <main className="fieldShell echoShell">
    <aside className="fieldHud">
      <button type="button" className="toolBrandHome" onClick={onHome} aria-label="Back to 1337tools home"><div className="brand"><span className="brand1337">1337</span><span className="brandTools">tools</span><small>4.25 / 05 ECHO</small></div></button>
