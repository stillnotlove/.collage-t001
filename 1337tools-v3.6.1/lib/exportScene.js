import {fx0,dotNoiseSpec,scratchSpec} from './effects';
...
function makeNoise(w,h,seed,amount){
  ...
  for(const p of dotNoiseSpec(seed,amount)){
    x.globalAlpha=p.opacity;x.fillStyle=p.dark?'#000':'#fff';x.beginPath();x.ellipse(p.x/100*w,p.y/100*h,p.r/100*w,p.r/100*h,0,0,Math.PI*2);x.fill();
  }
  x.globalAlpha=1;return c;
}
function makeScratches(w,h,seed,amount){
  const c=canvasOf(w,h),x=ctxOf(c),unit=Math.min(w,h)/100;
  for(const p of scratchSpec(seed,amount)){
    x.strokeStyle=p.color;x.globalAlpha=p.opacity;x.lineWidth=Math.max(.5,p.width*unit);x.lineCap=p.cap;x.beginPath();x.moveTo(p.x1/100*w,p.y1/100*h);x.lineTo(p.x2/100*w,p.y2/100*h);x.stroke();
  }
  x.globalAlpha=1;return c;
}
function makeHalftone(w,h){
  const c=canvasOf(w,h),x=ctxOf(c);x.fillStyle='#000';
  for(let py=4;py<100;py+=8)for(let px=4;px<100;px+=8){x.beginPath();x.ellipse(px/100*w,py/100*h,1.6/100*w,1.6/100*h,0,0,Math.PI*2);x.fill()}
  return c;
}
...
async function renderImageLayer(l,w,h,imageCache){
  ...
  if(f.noise>0){const e=makeNoise(w,h,seed,f.noise);maskEffect(e,alpha);compositeEffect(out,e,'soft-light',f.noise/105)}
...
export async function exportScene(layers,{kind='png',background='#fff',docWidth=1200,docHeight=1500}={}){
  ...
  const out=canvasOf(docWidth,docHeight),ctx=ctxOf(out);ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';ctx.fillStyle=background;ctx.fillRect(0,0,docWidth,docHeight);
