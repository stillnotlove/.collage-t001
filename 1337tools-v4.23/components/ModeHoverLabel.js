...
export default function ModeHoverLabel({mode,label,active}){
  ...
  useEffect(()=>{
    ...
    function drawAscii(ctx,p,time){
      ...
      const sample=makeCanvas(cols,rows),sx=sample.getContext('2d',{willReadFrequently:true});
      sx.clearRect(0,0,cols,rows);sx.drawImage(mask,0,0,cols,rows);
      const px=sx.getImageData(0,0,cols,rows).data;
      ctx.globalAlpha=1-p;ctx.drawImage(source,0,0);ctx.globalAlpha=p;ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillStyle='#0038ff';ctx.font=`${v.weight} ${cell*(v.id==='SQUARES'?1.06:1.12)}px IBM Plex Mono, Courier New, monospace`;
    ...
    function drawSlice(ctx,p){
      ...
      if(p>.35){ctx.save();ctx.globalAlpha=.12*p;ctx.fillStyle='#0038ff';ctx.fillRect(0,cutY-gap*.15,w,Math.max(1,gap*.3));ctx.restore()}
    }
    ...
    function drawEditor(ctx,p,time){
      ...
      ctx.save();ctx.strokeStyle='#0038ff';ctx.lineWidth=Math.max(1,1.25*dpr);ctx.globalAlpha=.3+.7*p;ctx.setLineDash([Math.max(3,5*dpr),Math.max(2,3*dpr)]);ctx.strokeRect(boxX,boxY,boxW,boxH);ctx.setLineDash([]);
