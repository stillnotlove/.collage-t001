...
export default function CollageEditor({initialFile=null,initialRatio='4 / 5',trail=[],onIndex,onHome}){
  ...
  function makeStamp(x,y,angle=0,order=1){
    ...
    if(stamp.type==='text')return {id:uid(),type:'text',fromStamp:true,stampScale:scale,text:stamp.text||'STAMP',x:px,y:py,width:d.width,height:d.height,rotation:rot,opacity,blend:'multiply',hidden:false,locked:false,order,color:stamp.color||'#111111',fontFamily:stamp.fontFamily||fonts[1][0],fontSize:58,fontWeight:stamp.fontWeight||700,tracking:+stamp.tracking||0,lineHeight:.9,scaleX:1,scaleY:1,outline:+stamp.outline||0,outlineColor:stamp.outlineColor||stamp.color||'#111111',outlineMode:'center'};
    return {id:uid(),type:'shape',fromStamp:true,stampScale:scale,shape:stamp.type,x:px,y:py,width:d.width,height:d.height,rotation:rot,opacity,blend:'multiply',hidden:false,locked:false,order,fill:'#111',stroke:'#111',strokeWidth:0,lineCap:'round'};
  }
  ...
  return <main className="shell">
    <aside className="panel left">
      <button type="button" className="toolBrandHome" onClick={onHome} aria-label="Back to 1337tools home"><div className="brand"><span className="brand1337">1337</span><span className="brandTools">tools</span><small>4.23 / 04 EDITOR</small></div></button>{onIndex&&<button className="wide fieldBack" onClick={onIndex}>← INDEX</button>}
