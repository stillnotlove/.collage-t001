...
check(shell.includes('<span>4.45</span>'),'INDEX: version is not 4.45');
check((shell.match(/4\.45/g)||[]).length===1,'INDEX: version should appear only once in AppShell');
...
check(entry.includes("phase='cleanToAsciiA'"),'ENTRY: clean → ASCII transition missing');
check(entry.includes("phase='asciiToSlice'"),'ENTRY: ASCII → SLICE transition missing');
check(entry.includes("phase='sliceToAsciiB'"),'ENTRY: SLICE → second ASCII transition missing');
...
check(entry.includes('renderSlice(sliceFrame')||entry.includes('renderSlice(target'),'ENTRY: SLICE renderer missing');
...
check(!entry.includes('const changed=resize();\n      if(changed)'),'ENTRY: per-frame resize layout read returned');
...
check(visualAudit.includes('<span>4.45</span>'),'QA: static visual audit has stale version');
...
check(pkg.version==='0.4.45','package.json: stale version');
