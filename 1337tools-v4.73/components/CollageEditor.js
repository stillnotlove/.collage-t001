...
import {isEditableTarget} from '../lib/browserUtils';
import ImageLayer from './layers/ImageLayer';
import ShapeLayer from './layers/ShapeLayer';
import TextLayer from './layers/TextLayer';
import RangeInputs from './RangeInputs';

const EDITOR_PNG_EXPORT_SCALE=2;

export default function CollageEditor({initialFile=null,initialRatio='4 / 5',trail=[],onIndex,onHome}){
  ...
  const [stamp,setStamp]=useState({type:'text',text:'1337',size:22,spacing:18,chaos:10,rotationJitter:20,scaleJitter:0,opacityJitter:0,follow:false,fontFamily:fonts[1][0],fontWeight:700,color:'#111111',tracking:0,outline:0,outlineColor:'#111111',fill:'#111111',stroke:'#111111',strokeWidth:0});
  const [marquee,setMarquee]=useState(null);

  const canvas=useRef(null),fileRef=useRef(null),gesture=useRef(null),before=useRef(null),brush=useRef(null),variationBase=useRef(null),controlBefore=useRef(null),ownedUrls=useRef(new Set()),keyActions=useRef(null),marqueeLive=useRef(null),suppressCanvasClick=useRef(false),initialLoaded=useRef(false),sceneRef=useRef(null);
  const doc=RATIOS[ratio]||RATIOS['4 / 5'];
  const processLabel=(trail?.length?trail:['EDITOR']).join(' → ');
  ...
  const multiBounds=useMemo(()=>getBounds(selectedLayers,doc.width/doc.height),[selectedLayers,doc.width,doc.height]);
  const imageEffectKeys=['threshold','grain','noise','halftone','xerox','fade'];

  sceneRef.current={layers,selectedIds,primaryId,ratio,bg};

  const currentSnap=()=>snap(layers,selectedIds,primaryId,{ratio,bg});
  const ownUrl=blob=>{const url=URL.createObjectURL(blob);ownedUrls.current.add(url);return url};
  const restore=s=>{setLayers(s.layers||[]);setSelectedIds(s.selectedIds||[]);setPrimaryId(s.primaryId||null);if(s.ratio&&RATIOS[s.ratio])setRatio(s.ratio);if(s.bg)setBg(s.bg);variationBase.current=null};
  ...
  function commit(next,nextSelection=selectedIds,nextPrimary=primaryId,{preserveVariation=false}={}){
    const prev=currentSnap(),after=snap(next,nextSelection,nextPrimary,{ratio,bg});
  ...
  function commitCurrent(next,nextSelection,nextPrimary){
    const s=sceneRef.current||{layers,selectedIds,primaryId,ratio,bg};
    const selection=nextSelection??s.selectedIds,primary=nextPrimary??s.primaryId;
    const prev=snap(s.layers,s.selectedIds,s.primaryId,{ratio:s.ratio,bg:s.bg}),after=snap(next,selection,primary,{ratio:s.ratio,bg:s.bg});
    if(prev!==after){setHistory(h=>[...h.slice(-39),prev]);setFuture([])}
    setLayers(next);setSelectedIds(selection);setPrimaryId(primary);variationBase.current=null;
  }
  ...
  function changeRatio(nextRatio){
    if(!RATIOS[nextRatio]||nextRatio===ratio)return;
    const oldAspect=doc.width/doc.height,nextDoc=RATIOS[nextRatio],nextAspect=nextDoc.width/nextDoc.height,factor=nextAspect/oldAspect;
    const nextLayers=layers.map(l=>({...l,height:clamp((+l.height||1)*factor,1,200)}));
    const prev=currentSnap(),after=snap(nextLayers,selectedIds,primaryId,{ratio:nextRatio,bg});
    if(prev!==after){setHistory(h=>[...h.slice(-39),prev]);setFuture([])}
    setLayers(nextLayers);setRatio(nextRatio);variationBase.current=null;
  }
  function changeBg(nextBg){
    if(!nextBg||nextBg===bg)return;
    const prev=currentSnap(),after=snap(layers,selectedIds,primaryId,{ratio,bg:nextBg});
    if(prev!==after){setHistory(h=>[...h.slice(-39),prev]);setFuture([])}
    setBg(nextBg);variationBase.current=null;
  }

  keyActions.current={selectAll,undo,redo,duplicate,remove,clearSelection:()=>setSelection([])};
  useEffect(()=>{
    const fn=e=>{
      const meta=e.metaKey||e.ctrlKey,editing=isEditableTarget(),a=keyActions.current;
      if(meta&&e.key.toLowerCase()==='a'&&!editing){e.preventDefault();a.selectAll()}
      if(meta&&e.key.toLowerCase()==='z'&&!editing){e.preventDefault();e.shiftKey?a.redo():a.undo()}
      if(meta&&e.key.toLowerCase()==='d'&&!editing){e.preventDefault();a.duplicate()}
      if((e.key==='Delete'||e.key==='Backspace')&&!editing){e.preventDefault();a.remove()}
      if(e.key==='Escape'&&!editing)a.clearSelection();
    };
  ...
  useEffect(()=>{if(initialRatio&&RATIOS[initialRatio])setRatio(initialRatio)},[initialRatio]);
  ...
  async function addFiles(list){
    ...
    for(const f of files){
      const src=ownUrl(f);
      try{
        const d=await imgSize(src),ar=d.width/d.height,currentRatio=sceneRef.current?.ratio||ratio,currentDoc=RATIOS[currentRatio]||doc,canvasAspect=currentDoc.width/currentDoc.height;
        let width=38,height=width*canvasAspect/ar;
        if(height>50){height=50;width=height*ar/canvasAspect}
        width=clamp(width,2,70);height=clamp(height,2,70);
        adds.push({id:uid(),type:'image',name:f.name,src,originalSrc:src,local:true,x:50,y:50,width,height,rotation:0,opacity:1,blend:'normal',hidden:false,locked:false,order:0,fit:'cover',radius:0,fx:{...fx0},fxSeed:Math.floor(Math.random()*1e9)});
      }catch{URL.revokeObjectURL(src);ownedUrls.current.delete(src)}
    }
    if(adds.length){
      const s=sceneRef.current||{layers};let order=nextOrder(s.layers);const ready=adds.map(a=>({...a,order:order++})),ids=ready.map(a=>a.id);
      commitCurrent([...s.layers,...ready],ids,ids.at(-1));
    }
  }
  ...
  function addShape(shape='rect'){
    const canvasAspect=doc.width/doc.height,width=shape==='line'?44:28,height=shape==='line'?7:width*canvasAspect;
    const l={id:uid(),type:'shape',shape,x:50,y:50,width,height,rotation:0,opacity:1,blend:'normal',hidden:false,locked:false,order:nextOrder(layers),fill:'#111111',stroke:'#111111',strokeWidth:0,lineCap:'round'};
    commit([...layers,l],[l.id],l.id);
  }
  function remove(){
  ...
  function stampTextAspect(style=stamp){
    const lines=String(style.text||'1337').split('\n'),c=document.createElement('canvas'),x=c.getContext('2d');
    x.font=`${style.fontWeight||700} 58px ${style.fontFamily||fonts[1][0]}`;
    const tracking=(+style.tracking||0)*.5;
    const widths=lines.map(line=>x.measureText(line||' ').width+Math.max(0,[...(line||' ')].length-1)*tracking);
    const width=Math.max(24,...widths),height=Math.max(52.2,lines.length*52.2);
    return clamp(width/height,.35,8);
  }
  function stampDims(style=stamp,scale=1,type=style.type){
    if(type==='text'){
      const width=clamp((+style.size||22)*scale,2,120),aspect=stampTextAspect(style);
      return {width,height:clamp(width*(doc.width/doc.height)/aspect,1,120)};
    }
    const width=clamp((+style.size||22)/2*scale,2,120),pixelAspect=type==='line'?5:1;
    return {width,height:clamp(width*(doc.width/doc.height)/pixelAspect,1,120)};
  }
  function updateStampedText(patch){
    const next={...stamp,...patch};setStamp(next);
    if(!selectedIds.length)return;
    setLayers(ls=>ls.map(l=>{
      if(!selectedIds.includes(l.id)||!l.fromStamp||l.type!=='text')return l;
      const style={...next,text:patch.text??l.text,fontFamily:patch.fontFamily??next.fontFamily,fontWeight:patch.fontWeight??next.fontWeight,tracking:patch.tracking??next.tracking};
      const d=stampDims(style,l.stampScale||1,'text');
      return {...l,text:style.text,fontFamily:style.fontFamily,fontWeight:style.fontWeight,tracking:style.tracking,color:patch.color??next.color,outline:patch.outline??next.outline,outlineColor:patch.outlineColor??next.outlineColor,width:d.width,height:d.height};
    }));
  }
  function updateStampSize(value){
    const next={...stamp,size:+value};setStamp(next);
    if(!selectedIds.length)return;
    setLayers(ls=>ls.map(l=>{
      if(!selectedIds.includes(l.id)||!l.fromStamp)return l;
      const d=stampDims({...next,text:l.text,fontFamily:l.fontFamily,fontWeight:l.fontWeight,tracking:l.tracking},l.stampScale||1,l.type==='text'?'text':l.shape);
      return {...l,width:d.width,height:d.height};
    }));
  }

  function updateStampedShape(patch){
    const next={...stamp,...patch};setStamp(next);
    if(!selectedIds.length)return;
    setLayers(ls=>ls.map(l=>{
      if(!selectedIds.includes(l.id)||!l.fromStamp||l.type!=='shape')return l;
      return {...l,fill:patch.fill??l.fill??next.fill,stroke:patch.stroke??l.stroke??next.stroke,strokeWidth:patch.strokeWidth??l.strokeWidth??next.strokeWidth};
    }));
  }
  ...
  function makeStamp(x,y,angle=0,order=1){
    ...
    const px=clamp(x+randSigned()*pos,-20,120),py=clamp(y+randSigned()*pos,-20,120),rot=angle+randSigned()*rotRange,scale=clamp(1+randSigned()*scaleRange,.18,2.4),opacity=clamp(1-Math.random()*opacityRange,.08,1),d=stampDims(stamp,scale,stamp.type);
    if(stamp.type==='text')return {id:uid(),type:'text',fromStamp:true,stampScale:scale,text:stamp.text||'1337',x:px,y:py,width:d.width,height:d.height,rotation:rot,opacity,blend:'multiply',hidden:false,locked:false,order,color:stamp.color||'#111111',fontFamily:stamp.fontFamily||fonts[1][0],fontSize:58,fontWeight:stamp.fontWeight||700,tracking:+stamp.tracking||0,lineHeight:.9,scaleX:1,scaleY:1,outline:+stamp.outline||0,outlineColor:stamp.outlineColor||stamp.color||'#111111',outlineMode:'center'};
    return {id:uid(),type:'shape',fromStamp:true,stampScale:scale,shape:stamp.type,x:px,y:py,width:d.width,height:d.height,rotation:rot,opacity,blend:'multiply',hidden:false,locked:false,order,fill:stamp.fill||'#111111',stroke:stamp.stroke||stamp.fill||'#111111',strokeWidth:Math.max(0,+stamp.strokeWidth||0),lineCap:'round'};
  }
  ...
  function move(e){
    ...
    if(g.kind==='groupRotate'){
      const a=Math.atan2(e.clientY-g.cy,e.clientX-g.cx),delta=(a-g.startAngle)*180/Math.PI,rad=delta*Math.PI/180,aspect=doc.width/doc.height;
      setLayers(ls=>ls.map(l=>{const s=g.starts[l.id];if(!s)return l;const dx=(s.x-g.b.cx)*aspect,dy=s.y-g.b.cy;return {...l,x:g.b.cx+(dx*Math.cos(rad)-dy*Math.sin(rad))/aspect,y:g.b.cy+dx*Math.sin(rad)+dy*Math.cos(rad),rotation:s.rotation+delta}}));return;
    }
  ...
  function up(){
    ...
    if(g.kind==='marquee'){
      const m=marqueeLive.current;if(m){const hits=layers.filter(l=>!l.hidden&&intersectsMarquee(l,m,doc.width/doc.height)).map(l=>l.id);const ids=g.add?[...new Set([...selectedIds,...hits])]:g.remove?selectedIds.filter(id=>!hits.includes(id)):hits;setSelection(ids,ids.at(-1)||null)}
  ...
  async function cutout(){
    ...
    const targetId=selected.id,source=selected.originalSrc||selected.src,seed=Math.floor(Math.random()*1e9);
    try{
      const blob=await experimentalCutout(source,seed),url=ownUrl(blob),s=sceneRef.current||{layers,selectedIds,primaryId};
      if(!s.layers.some(l=>l.id===targetId)){URL.revokeObjectURL(url);ownedUrls.current.delete(url);return}
      const next=s.layers.map(l=>l.id===targetId?{...l,src:url,originalSrc:l.originalSrc||l.src,cutout:true,cutoutSeed:seed}:l);
      commitCurrent(next,s.selectedIds,s.primaryId);
    }catch(err){console.error(err);alert('Remove BG failed for this image. Try a local image with a clearer background.')}finally{setCutting(false)}
  }
  ...
  async function exportImg(kind,transparent=false){
    if(kind==='jpg'&&transparent)transparent=false;
    setExporting(true);
    try{
      const blob=await exportScene(layers,{kind,background:bg,transparent,docWidth:doc.width,docHeight:doc.height,scale:kind==='png'?EDITOR_PNG_EXPORT_SCALE:1});
      const suffix=transparent&&kind==='png'?'transparent-':'';
      const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`1337tools-${suffix}${Date.now()}.${kind}`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1500);
  ...
  return <main className="shell">
    <aside className="panel left">
      <button type="button" className="toolBrandHome" onClick={onHome} aria-label="Back to 1337tools home"><div className="brand"><span className="brand1337"><span className="brandOneNudge">1</span>337</span><span className="brandTools">tools</span></div></button>{onIndex&&<button className="wide fieldBack" onClick={onIndex}>← INDEX</button>}
      ...
      {tool==='stamp'&&<HudGroup title="Stamp / Brush" storageKey="left-stamp" defaultOpen accent>
        ...
        {stamp.type==='text'&&<>
          <label>Word<input value={stamp.text} onFocus={beginControl} onBlur={endControl} onChange={e=>updateStampedText({text:e.target.value})}/></label>
          <label>Font<select value={stamp.fontFamily} onFocus={beginControl} onBlur={endControl} onChange={e=>updateStampedText({fontFamily:e.target.value})}>{fonts.map(([v,n])=><option value={v} key={n}>{n}</option>)}</select></label>
          <Range label="Weight" min="100" max="900" step="100" value={stamp.fontWeight} onBegin={beginControl} onEnd={endControl} onChange={v=>updateStampedText({fontWeight:+v})}/>
          <Range label="Tracking" min="-10" max="30" value={stamp.tracking} onBegin={beginControl} onEnd={endControl} onChange={v=>updateStampedText({tracking:+v})}/>
          <Range label="Outline" min="0" max="20" value={stamp.outline} onBegin={beginControl} onEnd={endControl} onChange={v=>updateStampedText({outline:+v})}/>
          <div className="two"><ColorControl label="Fill" value={stamp.color} onBegin={beginControl} onEnd={endControl} onChange={v=>updateStampedText({color:v})}/><ColorControl label="Outline" value={stamp.outlineColor} onBegin={beginControl} onEnd={endControl} onChange={v=>updateStampedText({outlineColor:v})}/></div>
        </>}
        {stamp.type!=='text'&&<>
          <div className="two"><ColorControl label="Fill" value={stamp.fill} onBegin={beginControl} onEnd={endControl} onChange={v=>updateStampedShape({fill:v})}/><ColorControl label="Stroke" value={stamp.stroke} onBegin={beginControl} onEnd={endControl} onChange={v=>updateStampedShape({stroke:v})}/></div>
          <Range label="Stroke width" min="0" max="20" value={stamp.strokeWidth} onBegin={beginControl} onEnd={endControl} onChange={v=>updateStampedShape({strokeWidth:+v})}/>
        </>}
        <Range label="Size" min="8" max="80" value={stamp.size} onBegin={beginControl} onEnd={endControl} onChange={updateStampSize}/>
      ...
      <HudGroup title="Canvas" storageKey="left-canvas">
        <label>Ratio<select value={ratio} onChange={e=>changeRatio(e.target.value)}>{Object.entries(RATIOS).map(([v,d])=><option value={v} key={v}>{d.label}</option>)}</select></label>
        <div className="note">Document: {doc.width} × {doc.height}px</div>
        <label>Background<div className="editorBgControl"><input type="color" value={bg} onPointerDown={beginControl} onFocus={beginControl} onBlur={endControl} onChange={e=>setBg(e.target.value)}/><input value={bg.toUpperCase()} readOnly aria-label="Canvas background hex"/></div></label>
        <div className="fieldBgPresets editorBgPresets"><button type="button" onClick={()=>changeBg('#f0ede4')}>PAPER</button><button type="button" onClick={()=>changeBg('#ffffff')}>WHITE</button><button type="button" onClick={()=>changeBg('#0038ff')}>BLUE</button><button type="button" onClick={()=>changeBg('#000000')}>BLACK</button></div>
      </HudGroup>
      ...
      <HudGroup title="System" storageKey="left-system">
        ...
        <div className="three"><button disabled={exporting} onClick={()=>exportImg('png',false)}>{exporting?'…':'PNG'}</button><button disabled={exporting} onClick={()=>exportImg('png',true)}>{exporting?'…':'PNG α'}</button><button disabled={exporting} onClick={()=>exportImg('jpg',false)}>{exporting?'…':'JPEG'}</button></div><div className="note">PNG α = export without background.</div>
      </HudGroup>
    </aside>

    <section className="workspace">
      <header className="topbar"><span>{layers.length} layers · {selectedIds.length} selected · {doc.width}×{doc.height}</span><b title={processLabel}>{processLabel}</b></header>
      <div className={`stage ${dragOver?'dragging':''}`} onDragOver={e=>{e.preventDefault();setDragOver(true)}} onDragLeave={()=>setDragOver(false)} onDrop={e=>{e.preventDefault();setDragOver(false);addFiles(e.dataTransfer.files)}}>
        <div ref={canvas} className={`canvas ${tool==='stamp'?'stampCursor':''}`} style={{aspectRatio:`${doc.width} / ${doc.height}`,'--canvas-bg':bg,'--doc-ratio':doc.width/doc.height}} onClick={canvasClick} onPointerDown={brushDown} onPointerMove={move} onPointerUp={up} onPointerLeave={up} onPointerCancel={up}>
          {!layers.length&&<div className="empty"><strong>DROP / STAMP / TEXT</strong><span>Build a composition, not a template.</span></div>}
...
function ImageLab({s,patch,patchFx,begin,end,cutting,cutout,restore,rerollAll}){
  ...
  return <>
    <HudGroup title="Image" storageKey="right-image-base" defaultOpen>
      ...
      <div className="two"><button className="primary" disabled={cutting} onClick={cutout}>{cutting?'REMOVING…':s.cutout?'RETRY BG β':'REMOVE BG β'}</button><button disabled={!s.originalSrc||!s.cutout} onClick={restore}>Restore</button></div>
      <div className="note">REMOVE BG β runs locally and is experimental. Restore keeps the original. Ctrl/Cmd+V pastes images from the clipboard.</div>
    </HudGroup>
...
function Range({label,value,onChange,onBegin,onEnd,...rest}){
  ...
  return <label className="range"><span>{label}</span><RangeInputs rowClass="rangeRow" numberClass="numberInput" min={min} max={max} step={step} value={value} onBegin={onBegin} onEnd={onEnd} onChange={onChange} onNumberDoubleClick={()=>label==='Rotate'&&onChange(0)}/></label>;
}
