...
import {experimentalCutout} from '../lib/effects';
...
export default function EchoTool({onIndex,onHome,onSendToEditor,onContinue,trail=[],initialFile,initialRatio='4 / 5',initialToken}){
  ...
  const [cutting,setCutting]=useState(false),[cutout,setCutout]=useState(false);
  const canvasRef=useRef(null),fileRef=useRef(null),imgRef=useRef(null),urlRef=useRef(null),loadSeq=useRef(0),cutoutSeq=useRef(0),originalFileRef=useRef(null);
  ...
  async function loadFile(file,{preserveOriginal=false,isCutout=false}={}){
    if(!file||!file.type?.startsWith('image/'))return false;
    if(!preserveOriginal)cutoutSeq.current++;
    const seq=++loadSeq.current,previous=urlRef.current,url=URL.createObjectURL(file),img=new Image();
    return await new Promise(resolve=>{
      img.onload=()=>{if(seq!==loadSeq.current){URL.revokeObjectURL(url);resolve(false);return}if(previous)URL.revokeObjectURL(previous);urlRef.current=url;imgRef.current=img;if(!preserveOriginal){originalFileRef.current=file;setSourceView(DEFAULT_VIEW)}setSource(url);setSourceName(file.name||'PASTED IMAGE');setSeed(Math.floor(Math.random()*1e9));setCutout(!!isCutout);resolve(true)};
      img.onerror=()=>{URL.revokeObjectURL(url);if(seq===loadSeq.current)reportToolError('ECHO image load',new Error('Unsupported or damaged image'));resolve(false)};
      img.src=url;
    });
  }

  async function removeBg(){
    if(!source||!imgRef.current||cutting)return;
    setCutting(true);const operation=++cutoutSeq.current;let tempUrl=null;
    try{
      // RETRY always starts from the true original source instead of repeatedly
      // cutting an already-cut PNG, which used to erode edges on every retry.
      let cutSource=source;
      if(cutout&&originalFileRef.current){tempUrl=URL.createObjectURL(originalFileRef.current);cutSource=tempUrl}
      const blob=await experimentalCutout(cutSource,Math.floor(Math.random()*1e9));
      if(operation!==cutoutSeq.current)return;
      const file=new File([blob],`ECHO-CUTOUT-${Date.now()}.png`,{type:'image/png'});
      await loadFile(file,{preserveOriginal:true,isCutout:true});
    }catch(err){reportToolError('ECHO remove BG',err)}finally{if(tempUrl)URL.revokeObjectURL(tempUrl);setCutting(false)}
  }
  async function restoreBg(){
    const file=originalFileRef.current;if(!file||cutting)return;
    await loadFile(file,{preserveOriginal:true,isCutout:false});
  }
  useEffect(()=>()=>{loadSeq.current++;cutoutSeq.current++;if(urlRef.current)URL.revokeObjectURL(urlRef.current)},[]);
  ...
  return <main className="fieldShell echoShell">
    <aside className="fieldHud">
      ...
      <MicroHudGroup title="INPUT" storageKey="echo-input" defaultOpen><button className="wide" onClick={()=>fileRef.current?.click()}>+ SOURCE IMAGE</button><input ref={fileRef} type="file" hidden accept="image/*" onChange={e=>{loadFile(e.target.files?.[0]);e.target.value=''}}/><div className="note">{sourceName}<br/>drop anywhere / Ctrl/Cmd+V</div>
        <EchoRange label="Image scale" min={25} max={400} value={sourceView.scale} onChange={v=>setView('scale',v)}/><EchoRange label="Image X" min={-100} max={100} value={sourceView.x} onChange={v=>setView('x',v)}/><EchoRange label="Image Y" min={-100} max={100} value={sourceView.y} onChange={v=>setView('y',v)}/><button className="wide" onClick={()=>setSourceView(DEFAULT_VIEW)}>RESET IMAGE VIEW</button><div className="two"><button className="primary" disabled={!source||cutting} onClick={removeBg}>{cutting?'REMOVING…':cutout?'RETRY BG β':'REMOVE BG β'}</button><button disabled={!cutout||cutting||!originalFileRef.current} onClick={restoreBg}>RESTORE</button></div><div className="note">REMOVE BG β runs locally and is experimental. Original source is kept for Restore.</div>
      </MicroHudGroup>
...
function EchoRange({label,value,onChange,min,max,step=1}){return <label className="fieldControl"><div className="fieldControlHead"><span>{label}</span></div><RangeInputs min={min} max={max} step={step} value={value} onChange={onChange}/></label>}
