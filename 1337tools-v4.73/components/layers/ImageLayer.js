...
import {fx0,dotNoiseBackground,grainBackground,halftoneBackground,xeroxBackground} from '../../lib/effects';
...
function ImageLayer({l}){
  ...
  return <div className="imageFrame" style={{borderRadius:`${l.radius||0}%`}}><img src={l.src} alt={l.name||''} draggable={false} style={{objectFit:l.fit||'cover',filter}}/><div className="fxMask" data-mask-src={l.src} style={mask}>
    {f.grain>0&&<i className="fx proceduralFx" style={{opacity:f.grain/150,backgroundImage:grainBackground(),mixBlendMode:'overlay'}}/>}
    {f.noise>0&&<i className="fx proceduralFx" style={{opacity:f.noise/90,backgroundImage:dotNoiseBackground(seed,f.noise),mixBlendMode:'soft-light'}}/>}
    {f.halftone>0&&<i className="fx proceduralFx" style={{opacity:(f.halftoneOpacity||85)/100*(f.halftone/100),backgroundImage:halftoneBackground(seed,f.halftone,{size:f.halftoneSize,density:f.halftoneDensity,angle:f.halftoneAngle,color:f.halftoneColor}),mixBlendMode:'multiply'}}/>}
    {f.xerox>0&&<i className="fx proceduralFx" style={{opacity:f.xerox/120,backgroundImage:xeroxBackground(),mixBlendMode:'multiply'}}/>}
