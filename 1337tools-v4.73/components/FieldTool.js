...
const FIELD_PNG_EXPORT_SCALE=2;
...
export default function FieldTool({onIndex,onHome,onSendToEditor,onContinue,trail=[],initialFile,initialRatio='4 / 5',initialToken}){
  ...
  function render(ctx,w,h,{clean=false,transparent=false}={}){
    ...
    const layer=document.createElement('canvas');layer.width=w;layer.height=h;const lx=layer.getContext('2d');lx.imageSmoothingEnabled=true;lx.imageSmoothingQuality='high';
    const src=processedSource(Math.max(w,h));
  ...
  async function renderBlob({transparent=false,scale=1}={}){const factor=Math.max(1,+scale||1),w=Math.max(1,Math.round(doc.w*factor)),h=Math.max(1,Math.round(doc.h*factor)),c=document.createElement('canvas');c.width=w;c.height=h;const ctx=c.getContext('2d');ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';render(ctx,w,h,{clean:true,transparent});return await new Promise((res,rej)=>c.toBlob(b=>b?res(b):rej(new Error('PNG encode failed')),'image/png'))}
  async function exportPng(transparent=false){if(exporting)return;setExporting(true);try{const blob=await renderBlob({transparent,scale:FIELD_PNG_EXPORT_SCALE});downloadBlob(blob,`1337-field-${transparent?'transparent-':''}${Date.now()}.png`)}catch(err){reportToolError('FIELD export',err)}finally{setExporting(false)}}
  async function sendToEditor(){if(!imgRef.current||exporting)return;setExporting(true);try{const blob=await renderBlob({transparent:false,scale:FIELD_PNG_EXPORT_SCALE}),file=new File([blob],`FIELD-${Date.now()}.png`,{type:'image/png'});onSendToEditor?.(file,ratio)}catch(err){reportToolError('FIELD → EDITOR',err)}finally{setExporting(false)}}
  async function continueTo(target){if(!imgRef.current||exporting)return;setExporting(true);try{const blob=await renderBlob({transparent:false,scale:FIELD_PNG_EXPORT_SCALE}),file=new File([blob],`FIELD-${target}-${Date.now()}.png`,{type:'image/png'});onContinue?.('field',target,file,ratio)}catch(err){reportToolError(`FIELD → ${String(target).toUpperCase()}`,err)}finally{setExporting(false)}}
