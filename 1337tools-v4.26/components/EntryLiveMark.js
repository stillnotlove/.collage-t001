...
const ROUTES=[
  ...
  ['outline','asciiSquare','rebuildSoft'],
  ['tiles','asciiBraille','rebuildGlyph'],
  ['outline','echo','asciiBlock','rebuildColumns'],
  ['tiles','distress','asciiSquare','rebuildSlice'],
];
const EFFECTS=['slice','ascii','asciiSquare','asciiBraille','asciiBlock','distress','echo','pixel','columns','outline','tiles'];
...
export default function EntryLiveMark(){
  ...
  useEffect(()=>{
    ...
    function frame(now){
      ...
      if(!route[stage]?.startsWith('rebuild')){ctx.save();ctx.globalAlpha=.045+.035*(1-p);ctx.drawImage(base,0,0);ctx.restore()}
