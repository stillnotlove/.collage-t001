...
export default function AppShell(){
  ...
  if(screen==='index')return <IndexScreen onOpen={openFromIndex} onHome={goHome}/>;
...
function IndexScreen({onOpen,onHome}){
  ...
  return <main className="indexScreen indexMinimal" data-active={active.id}>
    <header className="minimalHeader"><Brand onClick={onHome}/><span>4.26</span></header>
    <nav className="minimalModes" aria-label="1337tools">
      {MODES.map(m=><button key={m.id} type="button" className={`minimalMode ${m.id===hovered?'isHovered':''} isActive`} onMouseEnter={()=>setHovered(m.id)} onFocus={()=>setHovered(m.id)} onMouseLeave={()=>setHovered(null)} onBlur={()=>setHovered(null)} onClick={()=>onOpen(m.id)}>
        <span>{m.n}</span><strong className="modeLabelWrap"><ModeHoverLabel mode={m.id} label={m.name} active={m.id===hovered}/></strong><b>↗</b>
      </button>)}
    </nav>
    <div className="minimalGhost" aria-hidden="true"><span>{active.n}</span><b>{active.name}</b></div>
    <div className="indexCredit">by <b>stillnotlove</b></div>
  </main>;
}
