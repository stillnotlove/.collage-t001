...
export default function SliceTool({onIndex,onHome,onSendToEditor,onContinue,trail=[],initialFile,initialRatio='4 / 5',initialToken}){
  ...
  async function loadFile(file){
    ...
    return await new Promise(resolve=>{
      ...
      img.onerror=()=>{URL.revokeObjectURL(url);if(seq===loadSeq.current)reportToolError('SLICE image load',new Error('Unsupported or damaged image'));resolve(false)};
      img.src=url;
    });
  }
  ...
  return <main className="fieldShell sliceShell">
    <aside className="fieldHud">
      <button type="button" className="toolBrandHome" onClick={onHome} aria-label="Back to 1337tools home"><div className="brand"><span className="brand1337">1337</span><span className="brandTools">tools</span><small>4.26 / 02 SLICE</small></div></button>
