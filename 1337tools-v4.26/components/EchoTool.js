...
export default function EchoTool({onIndex,onHome,onSendToEditor,onContinue,trail=[],initialFile,initialRatio='4 / 5',initialToken}){
  ...
  const canvasRef=useRef(null),fileRef=useRef(null),imgRef=useRef(null),urlRef=useRef(null),loadSeq=useRef(0),originalFileRef=useRef(null);
  ...
  async function removeBg(){
    ...
    setCutting(true);let tempUrl=null;
    try{
  ...
  return <main className="fieldShell echoShell">
    <aside className="fieldHud">
      <button type="button" className="toolBrandHome" onClick={onHome} aria-label="Back to 1337tools home"><div className="brand"><span className="brand1337">1337</span><span className="brandTools">tools</span><small>4.26 / 05 ECHO</small></div></button>
