...
export default function EntryLiveMark(){
  ...
  useEffect(()=>{
    ...
    function transitionDuration(from,to){
      ...
      if(from.kind==='ascii'&&(to.kind==='slice'||to.kind==='echo'))return 1320+rnd()*520;
    ...
    function drawAsciiReveal(ctx,asciiFrame,p,seed,reverse=false){
      ...
      const mode=Math.floor(rnd()*3),horizontal=mode!==1,count=horizontal?14+Math.floor(rnd()*8):18+Math.floor(rnd()*8);
      for(let i=0;i<count;i++){
        const order=mode===2?Math.abs((i/Math.max(1,count-1))-.5)*1.9:(i/count);
        const stagger=rnd()*.24 + order*.14;
        const local=smootherstep(clamp01((p-stagger)/Math.max(.2,.74-stagger)));
        const alpha=reverse?1-local:local;
        if(alpha<=.008)continue;
        const drift=(1-local)*(rnd()-.5)*(horizontal?w*.012:h*.012);
    ...
    function overlapOutOfAscii(ctx,asciiFrame,motionFrame,p,seed){
      ...
      const motionIn=smootherstep(clamp01((p-.01)/.82));
      ctx.globalAlpha=.16+.84*motionIn;ctx.drawImage(motionFrame,0,0);
      drawAsciiReveal(ctx,asciiFrame,clamp01((p-.18)/.8),seed,true);
      ctx.globalAlpha=1;
    }
    ...
    function frame(now){
      ...
      if(phase==='hold')renderNode(nodes[nodeIndex],canvas,timeSec,1);
        ...
        const toIntensity=toIsMotion?(outOfAscii?lerp(.28,1,p):lerp(.34,1,p)):1;
