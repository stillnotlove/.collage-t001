...
check(shell.includes('<span>4.55</span>'),'INDEX: version is not 4.55');
check((shell.match(/4\.55/g)||[]).length===1,'INDEX: version should appear only once in AppShell');
...
check(entry.includes("['slice','ascii','echo']")&&entry.includes("['echo','ascii','slice']"),'ENTRY: ASCII bridge route grammar missing');
check(entry.includes('1366/Math.max(1,r.width)')&&entry.includes('820/Math.max(1,r.height)'),'ENTRY: backing-resolution cap missing');
check(entry.includes('requestIdleCallback'),'ENTRY: idle next-cycle preparation missing');
check(entry.includes('nextPrepared=buildCycle(cycleIndex+1)'),'ENTRY: next cycle is not prebuilt');
check(entry.includes('desc.cfg=makeSliceConfig(desc)'),'ENTRY: SLICE config caching missing');
check(entry.includes('desc.cfg=makeEchoConfig(desc)'),'ENTRY: ECHO config caching missing');
check(entry.includes('blendSource(sourceMix,base,to.frame,material)'),'ENTRY: motion-to-ASCII source morph missing');
check(entry.includes('blendSource(sourceMix,from.frame,base,material)'),'ENTRY: ASCII-to-motion source morph missing');
check(entry.includes('if(now-lastPaint<15.2)'),'ENTRY: high-refresh frame cap missing');
...
check(!entry.includes('compositeStripReveal'),'ENTRY: heavy strip compositor returned');
check(!entry.includes('renderSliceIntoAscii'),'ENTRY: stale dedicated slice->ASCII compositor returned');
check(!entry.includes('renderEchoIntoAscii'),'ENTRY: stale dedicated echo->ASCII compositor returned');
...
check(visualAudit.includes('<span>4.55</span>'),'QA: static visual audit has stale version');
...
check(pkg.version==='0.4.55','package.json: stale version');
