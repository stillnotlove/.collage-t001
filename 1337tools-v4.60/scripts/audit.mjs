...
check(shell.includes('<span>4.60</span>'),'INDEX: version is not 4.60');
check((shell.match(/4\.60/g)||[]).length===1,'INDEX: version should appear only once in AppShell');
...
check(css.includes('--brand-digit-gap:1px'),'BRAND: equal 1337 digit gap variable missing');
...
check(visualAudit.includes('<span>4.60</span>'),'QA: static visual audit has stale version');
...
check(pkg.version==='0.4.60','package.json: stale version');
