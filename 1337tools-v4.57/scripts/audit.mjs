...
check(shell.includes('<span>4.57</span>'),'INDEX: version is not 4.57');
check((shell.match(/4\.57/g)||[]).length===1,'INDEX: version should appear only once in AppShell');
...
check(entry.includes('function renderStaticAscii()'),'ENTRY: static ASCII renderer missing');
check(entry.includes("ctx.fillStyle='#1438ff'"),'ENTRY: blue splash background missing');
check(entry.includes("ctx.fillStyle='#fff'"),'ENTRY: white ASCII wordmark missing');
check(entry.includes("const chars=[...'@#%*+=:.']"),'ENTRY: ASCII charset missing');
check(entry.includes('ResizeObserver'),'ENTRY: resize observer guard missing');
check(!entry.includes('requestAnimationFrame'),'ENTRY: animated entry loop should be removed');
check(!entry.includes('requestIdleCallback'),'ENTRY: idle animation preparation should be removed');
check(!entry.includes('ROUTE_TEMPLATES'),'ENTRY: route animation system should be removed');
check(!entry.includes('SLICE_PATTERNS'),'ENTRY: motion animation patterns should be removed');
check(!entry.includes('ECHO_VARIANTS'),'ENTRY: motion animation variants should be removed');
...
check(visualAudit.includes('<span>4.57</span>'),'QA: static visual audit has stale version');
...
check(pkg.version==='0.4.57','package.json: stale version');
