...
check(shell.includes('<span>4.29</span>'),'INDEX: version is not 4.29');
check((shell.match(/4\.29/g)||[]).length===1,'INDEX: version should appear only once in AppShell');
check(shell.includes('className="minimalModes toolGrid"'),'INDEX: scalable tool grid missing');
check(shell.includes('className="catalogMeta"'),'INDEX: catalog metadata header missing');
...
check(shell.includes('all rights fucked'),'CREDITS: rights line missing');
check(shell.includes('design is your privilege'),'CREDITS: design line missing');
...
check(pkg.version==='0.4.29','package.json: stale version');
