...
export default function AppShell(){
  ...
  function openEditor(file=null,ratio='4 / 5'){setEditorFile(file);setEditorRatio(ratio||'4 / 5');setScreen('editor')}
  ...
  if(screen==='slice')return <SliceTool onIndex={()=>setScreen('index')} onSendToEditor={openEditor}/>;
  if(screen==='ascii')return <AsciiTool onIndex={()=>setScreen('index')} onSendToEditor={openEditor}/>;
  return <CollageEditor initialFile={editorFile} initialRatio={editorRatio} onIndex={()=>{setEditorFile(null);setEditorRatio('4 / 5');setScreen('index')}}/>;
}
...
function IndexScreen({onOpen}){
  ...
  return <main className="indexScreen indexMinimal" data-active={active.id}>
    <header className="minimalHeader"><Brand/><span>4.14</span></header>
