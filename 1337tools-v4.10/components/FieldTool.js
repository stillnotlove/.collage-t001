...
export default function FieldTool({onIndex,onSendToEditor}){
  ...
  const canvasRef=useRef(null),fileRef=useRef(null),imgRef=useRef(null),urlRef=useRef(null),fxCache=useRef({key:'',canvas:null});
  ...
  async function loadFile(file){
    ...
    const url=URL.createObjectURL(file);urlRef.current=url;fxCache.current={key:'',canvas:null};
    const img=new Image();
  ...
  // Important: bake FX into pixels instead of relying on ctx.filter.
  // This is consistent in Safari and in exported PNGs.
  function processedSource(maxSide=900){
    ...
    const exp=Math.pow(2,(+fx.exposure||0)/50),contrast=Math.max(0,(+fx.contrast||100)/100);
    ...
    const levels=Math.round(+fx.posterize||0);
  ...
  function drawNoise(ctx,w,h){
    ...
    for(let i=0;i<count;i++){
      ctx.fillStyle=rnd()>.5?'rgba(0,0,0,.28)':'rgba(255,255,255,.30)';
      const s=Math.max(.5,Math.min(w,h)*(.00045+rnd()*.0006));ctx.fillRect(rnd()*w,rnd()*h,s,s);
    }
    ctx.restore();
  }
  ...
  function render(ctx,w,h,{clean=false,transparent=false}={}){
    ...
    for(let gy=0;gy<rows&&drawn<count;gy++)for(let gx=0;gx<cols&&drawn<count;gx++,drawn++){
      ...
      ctx.save();ctx.translate(x,y);ctx.rotate(angle);ctx.globalAlpha=clamp(.28+near*.72+(rnd()-.5)*.42*chaos,.08,1);ctx.globalCompositeOperation=blendMode(fx.blend);ctx.drawImage(src,-iw/2,-ih/2,iw,ih);ctx.restore();
    }
    drawNoise(ctx,w,h);
  ...
  useEffect(()=>{
    const c=canvasRef.current;if(!c)return;const rect=c.getBoundingClientRect(),dpr=Math.min(2,window.devicePixelRatio||1);c.width=Math.max(1,Math.round(rect.width*dpr));c.height=Math.max(1,Math.round(rect.height*dpr));render(c.getContext('2d'),c.width,c.height);
  ...
  async function exportPng(transparent=false){if(exporting)return;setExporting(true);try{const blob=await renderBlob({transparent}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`1337-field-${transparent?'transparent-':''}${Date.now()}.png`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1500)}finally{setExporting(false)}}
  async function sendToEditor(){if(!imgRef.current)return;setExporting(true);try{const blob=await renderBlob({transparent:true}),file=new File([blob],`FIELD-${Date.now()}.png`,{type:'image/png'});onSendToEditor?.(file)}finally{setExporting(false)}}

  return <main className="fieldShell fieldShell41">
    <aside className="fieldHud">
      <div className="brand"><span className="brand1337">1337</span><span className="brandTools">tools</span><small>4.9 / 01 FIELD</small></div>
...
function SimpleRange({label,value,onChange,min,max}){return <label className="fieldControl"><div className="fieldControlHead"><span>{label}</span><b>{value}</b></div><div className="fieldRangeRow"><input type="range" min={min} max={max} value={value} onChange={e=>onChange(e.target.value)}/><input type="number" min={min} max={max} value={value} onChange={e=>onChange(e.target.value)}/></div></label>}
