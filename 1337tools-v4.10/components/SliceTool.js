...
export default function SliceTool({onIndex,onSendToEditor}){
  ...
  function render(ctx,w,h,{transparent=false}={}){
    ...
    pieces.forEach((dest,index)=>{
      ...
      ctx.save();ctx.translate(dx+dw/2,dy+dh/2);if(mirror)ctx.scale(params.mode==='ROWS'?-1:1,params.mode==='ROWS'?1:-1);ctx.globalAlpha=clamp(.72+rnd()*.28-(chaos*.12),.25,1);ctx.drawImage(src,s.sx,s.sy,s.sw,s.sh,-dw/2,-dh/2,dw,dh);ctx.restore();
  ...
  useEffect(()=>{const c=canvasRef.current;if(!c)return;const r=c.getBoundingClientRect(),dpr=Math.min(2,window.devicePixelRatio||1);c.width=Math.max(1,Math.round(r.width*dpr));c.height=Math.max(1,Math.round(r.height*dpr));render(c.getContext('2d'),c.width,c.height)},[source,params,seed]);
  ...
  async function exportPng(){if(exporting)return;setExporting(true);try{const b=await renderBlob(),u=URL.createObjectURL(b),a=document.createElement('a');a.href=u;a.download=`1337-slice-${Date.now()}.png`;a.click();setTimeout(()=>URL.revokeObjectURL(u),1500)}finally{setExporting(false)}}
  async function sendToEditor(){if(!imgRef.current)return;setExporting(true);try{const b=await renderBlob({transparent:true}),f=new File([b],`SLICE-${Date.now()}.png`,{type:'image/png'});onSendToEditor?.(f)}finally{setExporting(false)}}

  return <main className="fieldShell sliceShell">
    <aside className="fieldHud">
      <div className="brand"><span className="brand1337">1337</span><span className="brandTools">tools</span><small>4.9 / 02 SLICE</small></div>
      ...
      <section><div className="title">SLICE</div>
