...
export default function CollageEditor({initialFile=null,onIndex}){
  ...
  return <main className="shell">
    <aside className="panel left">
      <div className="brand" aria-label="1337tools"><span className="brand1337">1337</span><span className="brandTools">tools</span><small>4.9 / 03 EDITOR</small></div>{onIndex&&<button className="wide fieldBack" onClick={onIndex}>← INDEX</button>}
      ...
      {tool==='stamp'&&<HudGroup title="Stamp / Brush" storageKey="left-stamp" defaultOpen accent>
        ...
        {stamp.type==='text'&&<>
          <label>Word<input value={stamp.text} onChange={e=>updateStampedText({text:e.target.value})}/></label>
          <label>Font<select value={stamp.fontFamily} onChange={e=>updateStampedText({fontFamily:e.target.value})}>{fonts.map(([v,n])=><option value={v} key={n}>{n}</option>)}</select></label>
          <Range label="Weight" min="100" max="900" step="100" value={stamp.fontWeight} onChange={v=>updateStampedText({fontWeight:+v})}/>
          <Range label="Tracking" min="-10" max="30" value={stamp.tracking} onChange={v=>updateStampedText({tracking:+v})}/>
          <Range label="Outline" min="0" max="20" value={stamp.outline} onChange={v=>updateStampedText({outline:+v})}/>
          <div className="two"><ColorControl label="Fill" value={stamp.color} onChange={v=>updateStampedText({color:v})}/><ColorControl label="Outline" value={stamp.outlineColor} onChange={v=>updateStampedText({outlineColor:v})}/></div>
        </>}
        <Range label="Size" min="8" max="80" value={stamp.size} onChange={updateStampSize}/>
