...
check(shell.includes('<span>4.65</span>'),'INDEX: version is not 4.65');
check((shell.match(/4\.65/g)||[]).length===1,'INDEX: version should appear only once in AppShell');
...
check(entry.includes('<span className="brand1337">1337</span>'),'ENTRY: splash 1337 is still split into digit fragments');
...
check(shell.includes('<span className="brand1337">1337</span>'),'BRAND: AppShell wordmark is still split into digit fragments');
check(shell.includes('<b className="credits1337">1337</b>'),'CREDITS: credits 1337 is still split into digit fragments');
...
check(visualAudit.includes('<span>4.65</span>'),'QA: static visual audit has stale version');
...
check(layout.includes("'/favicon.ico?v=64'"),'ICONS: versioned favicon metadata missing');
check(layout.includes("'/icon-48.png?v=64'"),'ICONS: 48px search/browser icon missing from metadata');
...
check(manifest.includes("'/icon-48.png?v=64'"),'ICONS: 48px manifest icon missing');
...
check(layout.includes('<link rel="icon" href="/favicon.ico?v=64"'),'ICONS: explicit favicon link missing');
check(layout.includes('<link rel="shortcut icon" href="/favicon.ico?v=64"'),'ICONS: shortcut favicon link missing');
check(layout.includes('<link rel="apple-touch-icon" sizes="180x180" href="/apple-icon.png?v=64"'),'ICONS: explicit Apple touch icon missing');

const pkg=JSON.parse(read('package.json'));
check(pkg.version==='0.4.65','package.json: stale version');
