...
function LogoWord({className=''}){
  return <div className={`brand systemBrand ${className}`}><span className="brand1337">1337</span><span className="brandTools">tools</span></div>;
}
...
function IndexScreen({onOpen,onHome,onCredits}){
  ...
  return <main className="indexScreen indexMinimal catalogIndex" data-active={active.id}>
    <header className="minimalHeader"><Brand onClick={onHome}/><span>4.65</span></header>
...
function CreditsScreen({onBack,onHome}){
  return <main className="creditsScreen">
    ...
    <section className="creditsCenter">
      <div className="creditsTitle"><span className="creditsWordmark"><b className="credits1337">1337</b><i className="creditsTools">tools</i></span> <span className="creditsBy">by</span> <a href="https://www.instagram.com/stillnotlove?igsi=MWYyZmVxZDVnbHR4aw%3D%3D&utm_source=qr" target="_blank" rel="noreferrer">.stillnotlove</a></div>
