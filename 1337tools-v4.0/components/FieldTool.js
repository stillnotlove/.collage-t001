...
export default function FieldTool({onCompose,onSendToCompose}){
  ...
  function render(ctx,w,h,{clean=false}={}){
    ctx.clearRect(0,0,w,h);ctx.fillStyle='#f0ede4';ctx.fillRect(0,0,w,h);
  ...
  async function renderBlob(){
    const c=document.createElement('canvas');c.width=doc.w;c.height=doc.h;const ctx=c.getContext('2d');render(ctx,doc.w,doc.h,{clean:true});
  ...
  async function sendToCompose(){
    if(!imgRef.current)return;setExporting(true);try{const blob=await renderBlob();const file=new File([blob],`FIELD-${Date.now()}.png`,{type:'image/png'});onSendToCompose?.(file)}finally{setExporting(false)}
  }

  return <main className="fieldShell">
    <aside className="fieldHud">
      <div className="brand"><span className="brand1337">1337</span><span className="brandTools">tools</span><small>4.0 / FIELD</small></div>
      <div className="microNav"><button className="active">FIELD</button><button onClick={onCompose}>COMPOSE</button><button disabled>ECHO</button><button disabled>TYPE</button></div>

      <div className="fieldStatement">ONE SOURCE.<br/>ONE FIELD.<br/><b>KEEP REROLLING.</b></div>
      ...
      <section><div className="title">OUTPUT</div><button className="wide primary" disabled={!source||exporting} onClick={sendToCompose}>SEND TO COMPOSE</button><button className="wide" disabled={!source||exporting} onClick={exportPng}>{exporting?'RENDERING…':'EXPORT PNG'}</button></section>
    </aside>

    <section className="fieldWorkspace">
      <header className="fieldTop"><span>FIELD / {params.mode}</span><b>DRAG THE YELLOW POINT</b></header>
